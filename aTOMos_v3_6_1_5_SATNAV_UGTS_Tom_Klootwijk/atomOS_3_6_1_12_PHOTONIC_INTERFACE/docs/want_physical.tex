...
\end{equation}
Positive mass density and admissible basis fields linearly independent in the
corresponding weighted $L^2$ space give
\begin{equation}
 z^TMz=\int_{\Omega_g}\rho_m\left|\sum_i z_i\phi_i\right|^2dV>0
 \qquad(z\ne0).
\end{equation}
Thus the mass inverse exists. Coefficient integrals require explicit charts,
functions and Jacobians. For nested finite \code{INTEGRAL} nodes, every inner
section and each resulting outer integrand must be Riemann-integrable on
its declared finite bounds. This is an additional representability condition
on the chosen chart formula, not automatic support for all weak integrals.

Nonhomogeneous or moving supports require a lifting field and its additional
inertial and forcing terms. An analytic or modal basis does not require a
raster grid. Exact integration of the finite model still does not establish
that its selected basis represents every deformation of a real specimen.
...
\end{proof}
For $t\ge t_0$ and time-varying integrable nonnegative rates, the variation-of-constants
