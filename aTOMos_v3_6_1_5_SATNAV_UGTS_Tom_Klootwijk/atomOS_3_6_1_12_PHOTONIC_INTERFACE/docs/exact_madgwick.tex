...
\begin{center}\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{.34\linewidth}>{\raggedright\arraybackslash}X@{}}\toprule
...
Zero or missing acceleration & Set $e=0$ and use a gyro-only update; no residual, magnetic correction or drift adaptation is demanded.\\
...
Both matrices are declared proper orthogonal rotations: $C^TC=I$ and
$\det C=1$. Thus their transposes are their inverses. An arbitrary calibration
matrix with scale or shear does not satisfy this convention without a separate
coordinate model.
...
For unit $q$ and a fixed candidate derivative $G$, differentiating normalization
at $q$ gives
\[
 \mathcal N(q+hG)=q+h(I-qq^T)G+O(h^2).
\]
Thus the first-order vector field corresponding to this normalized step is
the projected candidate derivative. The gyro derivative is tangent, but the
source's ambient correction generally is not; identifying the continuous
unit-sphere evolution with the unprojected candidate would require a different
argument.
