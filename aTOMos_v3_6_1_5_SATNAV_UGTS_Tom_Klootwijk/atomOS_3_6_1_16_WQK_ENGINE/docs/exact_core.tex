...
This chapter restates the \code{XOP-R1} operator-seed contract introduced in
release 3.6.1.11 and adopted by the unified field profile. Definitions specify
a representation and its meaning;
...
subsets and their evidence are specified separately in the preceding chapters. The implemented R10 orbital seed and its binary64
...
\end{align*}
Here $\neg_w q=(2^w-1)\mathbin{\oplus}q$ and each word expression has
...
optimality. These are reasoning-based improvements in the retained XOP-R1 strategy
