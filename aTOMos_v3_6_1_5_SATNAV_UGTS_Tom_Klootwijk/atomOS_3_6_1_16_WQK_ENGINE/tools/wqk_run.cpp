...
const char* usage =
    ...
    "               [--output FILE.json]\n"
    "Defaults: lanes=1, steps=24, fetch=texture, feedback=none; JSON to stdout.\n"
    ...
    "One device batch, then final readback; no per-step host readback or trace.\n"
...
struct Options {
    ...
    U32 lanes = 1, steps = 24;
    bool texture = true, feedback = false;
};
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (options.lanes != 1)
            machine.set_states(std::vector<vm::State64>(options.lanes, machine.entry_state()));
        ...
        if (feedback) {
            feedback->run_steps(options.steps, options.texture);
            ...
            for (U32 step = 0; step < options.steps; ++step) machine.enqueue_step(options.texture);
        ...
        out << "{\n  \"profile\": \"ATOMOS-WQK-NATIVE-RUN-R1\",\n"
