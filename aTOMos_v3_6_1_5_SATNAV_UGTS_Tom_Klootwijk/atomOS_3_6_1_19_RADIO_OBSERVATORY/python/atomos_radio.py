"""R19 reported radio observations: exact byte archives and explicit study data.
...
def summary_markdown(summary) -> str:
    ...
    text = ["# aTOMos R19 radio study", "", f"Data origin: **{summary['data_origin']}**.", "",
...
def plot_signals(analysis, directory: Path):
    ...
    artifacts = []
    for index, (key, records) in enumerate(sorted(groups.items(), key=lambda pair: str(pair[0])), 1):
        ...
        for series_index, (label, points) in enumerate(sorted(series.items()), 1):
            basis = label[-1]
            ax.scatter([p[0] for p in points], [p[1] for p in points], s=30,
                       marker="o" if basis == "reported source time" else "x",
                       label=f"{identity_label(parse_json(label[2]))}; {label[0][:8]} / clock {label[1]}; {basis}")
        ...
        ax.set(title=f"{title_prefix}{radio_title} — {key[2]}",
               xlabel="Elapsed seconds from session/clock segment start (source or labeled receipt time)",
               ylabel=key[3])
        ax.grid(alpha=.25)
...
def study_file(source: Path, output: Path, *, plots=True, label=None, **policies):
    ...
    summary["plots"] = plot_signals(analysis, output) if plots else []
