"""Render R19 and prove pixel-identical preservation of every R18 page.

Run without confirmation first. After inspecting all seven supplement PNGs,
...
PDF = ROOT / "output/pdf/aTOMos_v3_6_1_19_Radio_Observatory_Tom_Klootwijk.pdf"
PARENT = ROOT.parent / "atomOS_3_6_1_18_SEEDED_EMIT_ENGINE/output/pdf/aTOMos_v3_6_1_18_Seeded_Emit_Engine_Tom_Klootwijk.pdf"
if not PARENT.exists():
    PARENT = ROOT / "source/parent_R18.pdf"
PARENT_SHA = "a62e3d7a57ec4c01c18308d9eb4b3c2f16172baf0e88276fbf6e64e591390cf0"
...
def main():
    ...
    assert build["source_sha256"] == sha(ROOT / "formal/RADIO_OBSERVATION_PROFILE.md"), "Formal source changed since build"
    poppler = shutil.which("pdftoppm")
    if not poppler:
        raise RuntimeError("pdftoppm is required for supplement visual rendering")
    RENDER.mkdir(parents=True, exist_ok=True)
    subprocess.run([poppler, "-f", "1", "-l", "7", "-r", "144", "-png",
                    str(PDF), str(RENDER / "supplement")], check=True,
                   stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    doc, parent = fitz.open(PDF), fitz.open(PARENT)
    assert len(doc) == 110 and len(parent) == 103, "Unexpected page count"
    inherited = []
    for index in range(len(parent)):
        original, original_bytes = pix_record(parent[index])
        retained, retained_bytes = pix_record(doc[index + 7])
        inherited.append({"parent_page": index + 1, "release_page": index + 8,
    ...
    for index in range(7):
        ...
        supplement.append({"page": index + 1, "png": str(image.relative_to(ROOT)),
                           ...
                           "header_present": "RADIO OBSERVATORY" in text,
                           "footer_present": "R19 supplement | Tom Klootwijk | 2026-09-17" in text,
    ...
    # exposes as NUL. They are present in the original reviewed R18 too and
    ...
    supplement_glyph_violations = sum(len(item[key]) for item in text_checks[:7]
    ...
    structure_ok = all(p["header_present"] and p["footer_present"] and p["page_number_present"]
                       for p in supplement)
    complete = bool(args.confirm_visual_sha256 and violations == 0 and inherited_count == 103 and structure_ok)
    report = {
        "schema": "atomos.r19.pdf_visual_review.v1",
        "reviewed_at_utc": datetime.now(timezone.utc).isoformat(),
        "pdf": str(PDF.relative_to(ROOT)), "sha256": pdf_sha,
        "source_sha256": build["source_sha256"], "pages": len(doc),
        "supplement_pages": 7, "parent_pages": 103, "parent_sha256": PARENT_SHA,
        ...
        "all_inherited_pages_pixel_equal": inherited_count == 103,
        ...
        "inherited_text_extraction_note": "Fourteen NUL extraction values from retained TeX Type1 extension glyphs also occur identically in the reviewed R18; every retained page is pixel-identical. They are not new missing visual glyphs.",
        "visual_review_sha256": args.confirm_visual_sha256,
        "visual_notes": ("All seven supplement pages inspected individually at rendered resolution: "
                         ...
                         "Pending inspection of all seven supplement PNGs."),
    ...
    if violations or inherited_count != 103 or not structure_ok:
