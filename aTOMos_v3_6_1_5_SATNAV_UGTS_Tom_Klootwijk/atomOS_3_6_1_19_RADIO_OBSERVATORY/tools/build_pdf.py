"""Build the editable R19 supplement and retain the exact reviewed R18 pages."""
...
PARENT = ROOT.parent / 'atomOS_3_6_1_18_SEEDED_EMIT_ENGINE'
PARENT_PDF = PARENT / 'output/pdf/aTOMos_v3_6_1_18_Seeded_Emit_Engine_Tom_Klootwijk.pdf'
if not PARENT_PDF.exists():
    PARENT_PDF = ROOT / 'source/parent_R18.pdf'
PARENT_SHA256 = 'a62e3d7a57ec4c01c18308d9eb4b3c2f16172baf0e88276fbf6e64e591390cf0'
NAME = 'aTOMos_v3_6_1_19_Radio_Observatory_Tom_Klootwijk.pdf'
...
def main():
    if sha(PARENT_PDF) != PARENT_SHA256:
        raise ValueError('R18 parent PDF hash mismatch')
    fonts()
    source = ROOT / 'formal/RADIO_OBSERVATION_PROFILE.md'
    input_hash = sha(source)
    out = ROOT / 'output/pdf'
    scratch = ROOT / 'tmp/pdf'
    out.mkdir(parents=True, exist_ok=True)
    scratch.mkdir(parents=True, exist_ok=True)
    supplement = scratch / 'r19_supplement.pdf'
    ...
    def page_frame(canvas, doc):
        ...
        canvas.drawString(48, 809, 'aTOMos  /  RADIO OBSERVATORY  /  3.6.1.19')
        canvas.setFont('Body', 8)
        canvas.drawString(48, 27, 'R19 supplement | Tom Klootwijk | 2026-09-17')
        canvas.drawRightString(547, 27, str(doc.page))
        canvas.restoreState()

    doc = SimpleDocTemplate(str(supplement), pagesize=A4, leftMargin=48, rightMargin=48,
                            topMargin=61, bottomMargin=47, title='aTOMos R19 Radio Observatory', author='Tom Klootwijk')
    doc.build(body, onFirstPage=page_frame, onLaterPages=page_frame)
    intro = PdfReader(supplement)
    parent = PdfReader(PARENT_PDF)
    if len(intro.pages) != 7 or len(parent.pages) != 103:
    ...
    writer.add_outline_item('R19 radio observation profile', 0)
    writer.add_outline_item('Retained R18 mathematical foundation (original pagination)', len(intro.pages))
    writer.add_metadata({'/Title': 'aTOMos 3.6.1.19 - Radio Observatory and Retained R18 Foundation', '/Author': 'Tom Klootwijk'})
    target = out / NAME
