...
Version label: aTOMos 3.6.1.7. Base-engine version: 3.6.1. Imported prototype:
...
CGK-R1 completes the connection through persistent ENU position/velocity, hoop and
time. COUPLED_CONTRACT.md defines the circle-plus blend, geometry-to-word encoders,
editable drive/X/J/K inputs, q-dependent stiffness and force, backward-Euler advance,
physical eigenmatrix and failure semantics. The actual default UGTS handoff executes
this native model and independently replays its trace before publishing the results.
The observation-only adapter is selected explicitly by --legacy-hold.
