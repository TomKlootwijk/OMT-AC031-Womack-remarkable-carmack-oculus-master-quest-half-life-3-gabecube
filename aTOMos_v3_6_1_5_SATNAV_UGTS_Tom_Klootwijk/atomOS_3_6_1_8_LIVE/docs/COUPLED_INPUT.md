...
python tools/coupled.py --input examples/coupled/model.json --out results/my_cpu --binary bin/windows/cpu/coupled_kernel.exe --backend cpu

# Execute and verify CUDA on device zero.
python tools/coupled.py --input examples/coupled/model.json --out results/my_cuda --binary bin/windows/cuda/coupled_kernel.exe --backend cuda --device 0
