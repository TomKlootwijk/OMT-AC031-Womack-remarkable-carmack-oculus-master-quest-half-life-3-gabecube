...
both 64-bit codecs and full coordinate/time records. Its observation adapter's
hold event remains that profile's explicit choice. SRK-R1 is the new formal word
state-feedback computation and runs without a GNSS residual admission condition.
The new bindings are identified as definitions of this release. The unavailable
earlier M2 circle-plus encoder and physical eigenmatrix are not asserted to have
been reconstructed from the surviving primitives.
\clearpage

\input{validation_current.tex}
