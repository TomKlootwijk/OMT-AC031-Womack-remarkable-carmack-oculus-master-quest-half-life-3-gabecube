...
public final class PacketDecoder {
    ...
    public static String decode(File file)throws IOException {
        State s=new State();s.fileBytes=file.length();
        ...
        if(file.length()!=s.fileBytes || !s.sha.equals(hash(file))){s.complete=false;s.status="source_changed";s.issue("capture bytes changed during inspection");}
    ...
    private static final class Reader {
        ...
        byte[] read(int n,boolean eof)throws IOException,Bad {
            ...
            while(p<n){int k=in.read(b,p,n-p);if(k<0){if(p==0&&eof)return null;throw new Bad("truncated capture at offset "+offset+"; needed "+(n-p)+" bytes");}p+=k;offset+=k;}
            return b;
        }
    }
    ...
    private static final class State {
        ...
        final JSONArray details=new JSONArray(),issues=new JSONArray(),interfaces=new JSONArray();
        ...
        JSONObject report(){return obj("schema","atomos.packet_inspection.v1","format",format,"status",status,"complete",complete,
        ...
        void packet(byte[] b,int link,long original,Object timestamp,int section,int interfaceId)throws Bad {
            ...
            try {primary=decodePacket(b,link,p,ls);if(primary.startsWith("Unsupported")){outcome="unsupported";unsupported++;issue("packet "+packets+": "+primary);}else decoded++;}
            ...
            if(details.length()<MAX_DETAILS)details.put(p);
        }
    }
    ...
    private static String decodePacket(byte[] b,int link,JSONObject p,JSONArray ls)throws Bad {
        int pos=0,ether=0;
        ...
        if(proto==17){ls.put("UDP");need(end-pos>=8,"UDP header truncated");int src=u16(b,pos,false),dst=u16(b,pos+2,false),n=u16(b,pos+4,false);need(n>=8,"invalid UDP length");put(p,"source_port",src);put(p,"destination_port",dst);put(p,"udp_length",n);boolean shortPayload=n>end-pos;put(p,"transport_payload_incomplete",shortPayload||fragment);int payloadEnd=Math.min(end,pos+n);
            boolean retained=p.optInt("index")<=MAX_DETAILS;if(retained)udpPreview(b,pos,payloadEnd,n,!shortPayload&&!fragment,p);
            if(!shortPayload&&!fragment){if(src==53||dst==53||src==5353||dst==5353){ls.put("DNS");put(p,"dns",dns(b,pos+8,payloadEnd));return "DNS";}
                if(retained){JSONObject application=udpApplication(b,pos+8,payloadEnd,src,dst);if(application!=null)put(p,"udp_application",application);}}
            return "UDP";}
    ...
    private static String hash(File f)throws IOException {try{MessageDigest d=MessageDigest.getInstance("SHA-256");try(InputStream in=new BufferedInputStream(new FileInputStream(f))){byte[] b=new byte[65536];int n;long total=0;while((n=in.read(b))!=-1){total+=n;if(total>MAX_FILE_BYTES)throw new IOException("capture grew beyond 64 MiB limit");d.update(b,0,n);}}return hex(d.digest());}catch(java.security.NoSuchAlgorithmException e){throw new AssertionError(e);}}
