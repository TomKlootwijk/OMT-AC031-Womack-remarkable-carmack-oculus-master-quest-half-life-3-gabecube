"""Render R20 and prove pixel-identical preservation of every R19 page.
...
PDF = ROOT / "output/pdf/aTOMos_v3_6_1_20_Radio_Packet_Studio_Tom_Klootwijk.pdf"
PARENT = ROOT.parent / "atomOS_3_6_1_19_RADIO_OBSERVATORY/output/pdf/aTOMos_v3_6_1_19_Radio_Observatory_Tom_Klootwijk.pdf"
if not PARENT.exists():
    PARENT = ROOT / "source/parent_R19.pdf"
PARENT_SHA = "0f95ef9fd27e4804a106d67e8a4d1b84325d25afd5e9be41b3e7e5d1701fa4ad"
...
def main():
    ...
    assert build["source_sha256"] == sha(ROOT / "formal/RADIO_PACKET_PROFILE.md"), "Formal source changed since build"
    ...
    assert len(doc) == 117 and len(parent) == 110, "Unexpected page count"
    inherited = []
    ...
    for index in range(7):
        ...
        supplement.append({"page": index + 1, "png": str(image.relative_to(ROOT)),
                           ...
                           "header_present": "RADIO PACKET STUDIO" in text,
                           "footer_present": "R20 supplement | Tom Klootwijk | 2026-09-17" in text,
    ...
    # exposes as NUL. They are present in the original reviewed R19 too and
    ...
    structure_ok = all(p["header_present"] and p["footer_present"] and p["page_number_present"]
                       for p in supplement)
    complete = bool(args.confirm_visual_sha256 and violations == 0 and inherited_count == 110 and structure_ok)
    report = {
        "schema": "atomos.r20.pdf_visual_review.v1",
        "reviewed_at_utc": datetime.now(timezone.utc).isoformat(),
        "pdf": str(PDF.relative_to(ROOT)), "sha256": pdf_sha,
        "source_sha256": build["source_sha256"], "pages": len(doc),
        "supplement_pages": 7, "parent_pages": 110, "parent_sha256": PARENT_SHA,
        ...
        "all_inherited_pages_pixel_equal": inherited_count == 110,
        ...
        "inherited_text_extraction_note": "Fourteen NUL extraction values from retained TeX Type1 extension glyphs also occur identically in the reviewed R19; every retained page is pixel-identical. They are not new missing visual glyphs.",
    ...
    if violations or inherited_count != 110 or not structure_ok:
