#!/usr/bin/env python3
"""Run the coupled native UGTS adapter; --legacy-hold selects the old observation adapter."""
...
def process_legacy_hold(input_dir,run_dir):
...
def process(input_dir, run_dir, *, legacy_hold=False, **kwargs):
    if legacy_hold:
        if kwargs.get('out') is not None:
            raise ValueError('--out is available for the coupled adapter; legacy writes to --run')
        return process_legacy_hold(Path(input_dir), Path(run_dir))
    from coupled_handoff import process as process_coupled
    return process_coupled(Path(input_dir), Path(run_dir), **kwargs)
...
if __name__=='__main__':
    ...
    p.add_argument('--legacy-hold',action='store_true',help='explicitly use the preserved 3.6.1.6 observation-only adapter')
    p.add_argument('--out',type=Path)
    p.add_argument('--backend',choices=('cpu','cuda'),default='cpu')
    p.add_argument('--binary',type=Path)
    p.add_argument('--device',type=int,default=0)
    p.add_argument('--coupled-profile',type=Path)
    a=p.parse_args()
    try:
        summary=process(a.input,a.run,legacy_hold=a.legacy_hold,out=a.out,backend=a.backend,
                        binary=a.binary,device=a.device,coupled_profile=a.coupled_profile)
