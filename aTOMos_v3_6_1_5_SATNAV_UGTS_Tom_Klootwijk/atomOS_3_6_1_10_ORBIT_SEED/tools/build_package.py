...
def delivered_files():
    for directory, names, files in os.walk(ROOT):
        ...
        pdf_review = len(relative.parts) >= 2 and relative.parts[0] == 'results' and relative.parts[1].startswith('orbit_pdf')
        names[:] = sorted(name for name in names if not (
            name.startswith('build') or name in {'.build', '__pycache__', '.git', 'tmp', 'LOCAL_APPDATA_FONTCONFIG_CACHE'} or
            (pdf_review and name in {'low', 'full', 'before_calendar_low'})))
