...
enum Status : unsigned { OK=0, OUTSIDE_FORCING=1, NUMERIC_FAILURE=2, WORK_LIMIT=3, OUTSIDE_DOMAIN=4, INSIDE_REFERENCE_EARTH=5 };
...
struct Model {
 ...
 double domain_start{},domain_end{};unsigned domain_declared{};
 Series q{},sun{},moon{},eop{};
};
struct Query { double time{},state[6]{}; std::uint64_t steps{}; unsigned status{},reused{}; };
struct Words {
...
SAT_HD inline unsigned time_status(const Model& m,double t) {
 if(!satnav::finite(t))return NUMERIC_FAILURE;
 return m.domain_declared&&(t<m.domain_start||t>m.domain_end)?OUTSIDE_DOMAIN:OK;
}
...
SAT_HD inline bool frame(const Model& m,double t,double* result) {
 if(time_status(m,t)!=OK)return false;
...
SAT_HD inline unsigned state_status(const Model& m,double t,const double* state) {
 const auto status=time_status(m,t);if(status)return status;
 if(!finite(state,6))return NUMERIC_FAILURE;
 double rot[9],r[3]{};if(!frame(m,t,rot))return OUTSIDE_FORCING;
 for(int i=0;i<3;++i)for(int j=0;j<3;++j)r[i]+=rot[3*i+j]*state[j];
 const double radius=norm(r);if(!satnav::finite(radius))return NUMERIC_FAILURE;
 return radius<=m.radius?INSIDE_REFERENCE_EARTH:OK;
}
...
SAT_HD inline unsigned acceleration(const Model& m,double t,const double* state,double* out) {
 const auto time_check=time_status(m,t);if(time_check)return time_check;
 if(!finite(state,6))return NUMERIC_FAILURE;
 double rot[9],r[3];if(!frame(m,t,rot))return OUTSIDE_FORCING;
 ...
 if(d<=m.radius)return INSIDE_REFERENCE_EARTH;
...
SAT_HD inline long long lattice_target(const Model& m,double t) {
 long long index=static_cast<long long>(t/m.step);
 // Division can round across an integer. Never place a predecessor beyond t.
 if(t>=0&&index*m.step>t)--index;
 if(t<0&&index*m.step<t)++index;
 return index;
}
SAT_HD inline unsigned rk4(const Model& m,double t,double h,double* state,double endpoint) {
 // Stage labels use the caller's exact lattice/request endpoint. In particular,
 // n*h+h need not equal (n+1)*h for decimal h at a closed domain boundary.
 double middle=t+h/2;
 if(h>=0){if(middle<t)middle=t;if(middle>endpoint)middle=endpoint;}
 else {if(middle>t)middle=t;if(middle<endpoint)middle=endpoint;}
 double k1[6],k2[6],k3[6],k4[6],temp[6];unsigned status=derivative(m,t,state,k1);if(status)return status;
 for(int i=0;i<6;++i)temp[i]=state[i]+h*k1[i]/2;status=derivative(m,middle,temp,k2);if(status)return status;
 for(int i=0;i<6;++i)temp[i]=state[i]+h*k2[i]/2;status=derivative(m,middle,temp,k3);if(status)return status;
 for(int i=0;i<6;++i)temp[i]=state[i]+h*k3[i];status=derivative(m,endpoint,temp,k4);if(status)return status;
 for(int i=0;i<6;++i)temp[i]=state[i]+(h/6)*(k1[i]+2*k2[i]+2*k3[i]+k4[i]);
 status=state_status(m,endpoint,temp);if(status)return status;
...
SAT_HD inline Query from_epoch(const Model& m,double t) {
 ...
 q.status=time_status(m,t);if(q.status)return q;
 if(!satnav::finite(t)||::fabs(t/m.step)>static_cast<double>(m.max_steps)){q.status=WORK_LIMIT;return q;}
 q.status=state_status(m,0,q.state);if(q.status)return q;
 const long long target=lattice_target(m,t),direction=target<0?-1:1;
 for(long long n=0;n!=target;n+=direction){q.status=rk4(m,n*m.step,direction*m.step,q.state,(n+direction)*m.step);++q.steps;if(q.status)return q;}
 const double remainder=t-target*m.step;if(remainder!=0){q.status=rk4(m,target*m.step,remainder,q.state,t);++q.steps;}return q;
}
} // namespace orbit
