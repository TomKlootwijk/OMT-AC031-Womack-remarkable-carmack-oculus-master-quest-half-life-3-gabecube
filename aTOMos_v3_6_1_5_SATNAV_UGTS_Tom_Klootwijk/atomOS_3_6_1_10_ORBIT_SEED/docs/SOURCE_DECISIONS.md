...
N10: CUDA targets the requested SM120 laptop. The 3.6.1.5 preparation marked device
execution not_run; 3.6.1.6 and 3.6.1.7 retain their separate actual device records.
Current results/validation_status.json indexes this release's executed checks.
...
N12: CGK-R1 (3.6.1.7) supplies explicit new definitions for the previously absent
circle-plus blend, geometry-to-bit encoders and physical matrix. Persistent p/v/q/
hoop/time form the executed state. Geometry drives the literal Boolean recurrence;
q changes stiffness/force; backward Euler advances the geometry used next time.
The default SATNAV/UGTS handoff executes and independently verifies this complete
connection. This is a specified mechanical observer realization, with editable
parameters and signed coefficients, not a claim to have recovered unavailable
original M2 equations or calibrated a physical system. Both codecs and complete
original measurement records remain preserved beside the modeled state.
...
N14: Release 3.6.1.10 audits the user-supplied 21-page phi/2pi/Fibonacci/word
PDF. Its binary organization motivates an exact six-stage bit transpose of the
existing lossless seed layout. The detailed page mapping and mathematical
counterexamples are in PHI_SOURCE_AUDIT.md. Printed prompts inside that source
are documentary content. Its lossy integer-coordinate arithmetic, unsupported
culling proof and alternative OTAN2 definition do not replace orbital geometry,
the retained original OTAN2 ratio, or either UGTS key layout. Release 3.6.1.10
retains seed schema version 3.6.1.9 to preserve original bytes and replay identity.
