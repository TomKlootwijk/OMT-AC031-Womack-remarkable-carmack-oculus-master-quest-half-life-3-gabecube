...
In the 3.6.1.10 runtime, each `NativeOrbit` and `OrbitSession` owns a frozen construction snapshot. A session accepts only a physical-model digest matching its native worker. Its public `seed` and `station` views are read-only; changing the original input dictionary cannot change the active trace, geometry or identity. To edit feedback, station or query settings, use `edited = session.copy_seed()`, modify that ordinary dictionary, and construct `OrbitSession(edited, worker)`. This creates a new seed identity and trace. The same worker can be reused when the physical model is unchanged; a physical-model change requires a new worker. Mid-trace edits are not silently applied.

Preparation retains exact typed content: station origins/rotations are cached by the binary64 coordinate bits, including signed zero, and feedback ASTs/truth tables are cached by the complete canonical feedback bytes. Standalone helper calls therefore notice every relevant edit and reject invalid types even after a prior cache hit. A query evaluates the frame and its derivative together, sharing their intermediate Q/EOP work only within that call; it does not cache a mutable model by object identity. These changes preserve the equations and numeric outputs.
