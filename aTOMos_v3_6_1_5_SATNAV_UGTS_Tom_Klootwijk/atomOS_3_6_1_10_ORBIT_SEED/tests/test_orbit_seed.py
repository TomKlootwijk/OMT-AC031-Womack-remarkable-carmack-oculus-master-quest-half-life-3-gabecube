...
class SeedTests(unittest.TestCase):
    def test_transpose_all_basis_vectors(self):
        # Off-diagonal bits distinguish a transpose from identity, bit reversal,
        # and a row/column orientation error. XOR-linearity extends this basis
        # check to every 4096-bit block.
        for row in range(64):
            for column in range(64):
                words, expected = [0] * 64, [0] * 64
                words[row], expected[column] = 1 << column, 1 << row
                data = words_to_bitplanes(words)
                self.assertEqual(data, struct.pack("<64Q", *expected))
                self.assertEqual(bitplanes_to_words(data, 64), words)

    def test_bitplane_public_encoder_respects_decoder_count_limit(self):
        with self.assertRaises(ValueError):
            words_to_bitplanes([0] * 300001)
