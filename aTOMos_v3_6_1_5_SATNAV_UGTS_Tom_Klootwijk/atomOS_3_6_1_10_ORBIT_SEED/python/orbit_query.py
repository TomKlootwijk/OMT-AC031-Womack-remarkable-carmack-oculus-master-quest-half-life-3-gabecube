...
def _station_transform(coordinate_bits):
    """Cache exact coordinate content, including signed zero, not station identity."""
    latitude, longitude, height = struct.unpack("<3d", coordinate_bits)
    lat, lon = math.radians(latitude), math.radians(longitude)
    origin = np.asarray(geodetic_to_ecef(lat, lon, height))
...
def station_geometry(model, time_s, state, station):
    ...
    origin, matrix = _station_transform(struct.pack("<3d", station["lat_deg"], station["lon_deg"], station["height_m"]))
...
class OrbitSession:
    """One immutable seed snapshot and identity per trace.

    To edit feedback, station or query settings, construct a new session from the
    edited seed. The same native worker may be reused when its model is unchanged.
    """
    def __init__(self, seed, worker, station_id=None):
        self._seed, self.worker = deepcopy(seed), worker
        self.seed_hash, self.model_hash = digest(self._seed), digest(self._seed["model"])
        if self.model_hash != getattr(worker, "model_sha256", None):
            raise ValueError("Session physical model differs from the frozen native model")
        stations = self._seed["stations"]
        self._station = next((s for s in stations if s["id"] == (station_id or stations[0]["id"])), None)
        if self._station is None: raise ValueError("Unknown station ID")
        self._seed_view, self._station_view = _readonly(self._seed), _readonly(self._station)

    @property
    def seed(self): return self._seed_view

    @property
    def station(self): return self._station_view

    def copy_seed(self):
        """Return an editable copy for constructing a new session and trace."""
    ...
    def query(self, time_s):
        ...
        state = _state(native)
        geometry = station_geometry(self._seed["model"], time_s, state, self._station)
        gpst = self._seed["model"]["epoch_gpst_s"] + time_s
        return {"seed_sha256": self.seed_hash, "model_sha256": self.model_hash,
                "object_id": self._seed["object"]["id"], "station_id": self._station["id"],
                "time_s": time_s, "time_gpst_s": gpst, "timestamp_gpst": timestamp_gpst(gpst),
                "state_gcrs_m_mps": state, **geometry, "chart": chart_record(self._seed, time_s, geometry),
    ...
    def schedule(self, start=None, end=None, max_records=100000):
        ...
        seed, feedback = self._seed, self._seed["feedback"]
        ...
        while True:
            ...
            record = self.query(t)
            drive = predicate_word(feedback, record, t, self._station)
