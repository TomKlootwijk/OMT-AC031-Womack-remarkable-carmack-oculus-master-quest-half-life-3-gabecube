...
def frame_matrix(model, t, derivative=False, *, _pair=False):
    if model['profile']=='ORBIT-DYNAMICS-R2':
        from orbit_precision import precision_frame
        return precision_frame(model,t,derivative,_pair=_pair)
    ...
    if not derivative and not _pair:
        return p@r@q
    dr=f['era_rate_rad_s']*np.array([[-s,c,0],[-c,-s,0],[0,0,0]])
    dq=segment_value(f['q_segments'],t,True).reshape(3,3)
    rate = p@(dr@q+r@dq)
    return (p@r@q, rate) if _pair else rate


def frame_pair(model, t):
    """Evaluate M and Mdot together; no model values persist beyond this call."""
    return frame_matrix(model, t, True, _pair=True)


def state_to_ecef(model, t, y):
    y=np.asarray(y,dtype=float); m, rate=frame_pair(model,t)
    return np.concatenate((m@y[:3],m@y[3:]+rate@y[:3]))
