...
def _readonly(value):
    ...
    if isinstance(value, list): return tuple(_readonly(item) for item in value)
    return value


class NativeOrbit:
    def __init__(self, worker_path, model, backend="cpu"):
        self._model, self.backend = deepcopy(validate_model(model)), backend
        self._model_sha256 = digest(self._model)
        self._model_view = _readonly(self._model)
        self._domain = tuple(self._model["domain_s"])
        ...
        self._model_path.write_text(numeric_model(self._model), encoding="ascii")
    ...
    @property
    def model(self): return self._model_view

    @property
    def model_sha256(self): return self._model_sha256

    def copy_model(self):
        """Return editable input for constructing a separate native worker."""
