...
# A 64 x 64 transpose exchanges the six row/column index bits.  In stage s,
# pair (row, column) with (row+s, column-s) when row&s == 0 and column&s != 0.
# A Python integer is an exact multiword temporary; the on-disk lanes remain uint64.
_TRANSPOSE_STAGES = tuple(
    (63 * shift, sum(sum(1 << column for column in range(64) if column & shift)
                     << (64 * row) for row in range(64) if not row & shift))
    for shift in (32, 16, 8, 4, 2, 1)
)
_BLOCK = struct.Struct("<64Q")


def _transpose64(block):
    """Exact involution: input bit (row, column) becomes (column, row)."""
    value = int.from_bytes(block, "little")
    for distance, mask in _TRANSPOSE_STAGES:
        exchange = (value ^ (value >> distance)) & mask
        value ^= exchange ^ (exchange << distance)
    return value.to_bytes(512, "little")


def words_to_bitplanes(numbers):
    """Plane b, group g stores bit b of up to 64 original words, lane j%64."""
    if len(numbers) > 300000:
        raise ValueError("Invalid bit-plane word count")
    ...
    for group in range(groups):
        words = list(numbers[group * 64:(group + 1) * 64])
        words.extend([0] * (64 - len(words)))
        planes[group::groups] = _BLOCK.unpack(_transpose64(_BLOCK.pack(*words)))
...
def bitplanes_to_words(data, count):
    ...
    numbers = []
    for group in range(groups):
        block = _BLOCK.pack(*planes[group::groups])
        numbers.extend(_BLOCK.unpack(_transpose64(block)))
...
def _prepared_feedback(content):
    """Validate a typed content snapshot once; no mutable caller values are kept."""
    reader = _Reader(content)
    feedback = reader.value()
    if reader.offset != len(content): raise ValueError("Trailing feedback content")
    validate_feedback(feedback)
    expressions = tuple(parse_expression(feedback["equations"][key], variables)
                        for key, variables in VARIABLES.items())
    masks = tuple(tuple(stage[key] for key in ("asa_mask", "na_mask", "boundary_mask"))
                  for stage in feedback["sets"])
    return (feedback["present"], masks, expressions,
            tuple(compile_lut(expression) for expression in expressions))


def feedback_luts(feedback):
    prepared = _prepared_feedback(_encode(feedback))
    return dict(zip((key + "_lut" for key in VARIABLES), prepared[3]))


def transition_ast(feedback, q, drive):
    """Independent expression replay; both stages preserve whole-word absorption."""
    present, masks, expressions, _ = _prepared_feedback(_encode(feedback))
    ...
    x = expressions[0].evaluate({"q": q, "d": drive})
    value, stages = x, []
    for asa_mask, na_mask, boundary_mask in masks:
        asa = value & asa_mask & present
        na = asa & na_mask
        hits = (na & boundary_mask).bit_count()
        output = 0 if hits else na
        stages.append({"input": value, "asa": asa, "na": na, "hits": hits, "output": output})
        value = output
    values = {"q": q, "y": value, "d": drive}
    j, k = expressions[1].evaluate(values), expressions[2].evaluate(values)
