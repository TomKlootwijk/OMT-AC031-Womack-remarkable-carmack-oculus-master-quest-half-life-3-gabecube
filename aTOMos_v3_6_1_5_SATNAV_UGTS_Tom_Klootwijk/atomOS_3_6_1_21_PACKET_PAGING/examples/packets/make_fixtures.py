...
def query(qtype=1, name="example.test"):
    return struct.pack("!6H", 0x1919, 0x0100, 1, 0, 0, 0) + dns_name(name) + struct.pack("!HH", qtype, 1)
...
def build():
    ...
    paging = []
    for frame in range(1, 751):
        if frame % 3 == 0:
            packet = ether(ip4(tcp(b"", 40000 + frame, 49152, frame, 0, 2,
                "192.0.2.10", "198.51.100.20"), 6, ident=frame))
        else:
            data = query(name=f"page{frame:04d}.example.test") if frame in (452, 749) else f"SYNTHETIC-PAGE-{frame:04d}".encode("ascii")
            packet = ether(ip4(udp(data, 49000, 53 if frame in (452, 749) else 49001,
                "192.0.2.10", "198.51.100.20"), 17, ident=frame))
        paging.append((packet, len(packet)))
    files["synthetic_paging_750.pcap"] = classic(paging)
    manifest = {"synthetic": True, "network_calls": False, "note": "Documentation addresses and invented payloads. TLS bytes exercise record/ClientHello parsing, not an authenticated connection.",
        ...
        "paging_fixture": {"file": "synthetic_paging_750.pcap", "frames": 750,
            "tcp_frames": 250, "udp_frames_including_dns": 500,
            "rule": "One-based frames divisible by three are TCP SYN; every other frame is UDP. Only frames 452 and 749 use UDP port 53 with DNS questions.",
            "dns_frames": [452, 749], "dns_names": ["page0452.example.test", "page0749.example.test"],
            "dns_zero_based_udp_offsets": [301, 499],
            "udp_page_200": [{"offset": 0, "count": 200, "first_frame": 1, "last_frame": 299, "has_next": True},
                {"offset": 200, "count": 200, "first_frame": 301, "last_frame": 599, "has_next": True},
                {"offset": 400, "count": 100, "first_frame": 601, "last_frame": 749, "has_next": False}]},
        "files": []}
