...
public final class RadioService extends Service {
    ...
    private void startRecording() {
        try {
            ...
            append("session",null,Json.object("app","aTOMos Radio","app_version","3.6.1.21","device_manufacturer",Build.MANUFACTURER,"device_model",Build.MODEL,"android_release",Build.VERSION.RELEASE,"android_sdk",Build.VERSION.SDK_INT,
            ...
            if(log==null)return;
            networkState=null; captureNetwork(); setupWifi(); setupCellular(); handler.post(cellTick); handler.post(wifiTick);
    ...
    private void captureWifi(String origin,Boolean updated) {
        try {
            ...
            for(ScanResult r:results) {
                ...
                if(Build.VERSION.SDK_INT>=30 && r.getInformationElements()!=null)for(ScanResult.InformationElement ie:r.getInformationElements()) {
                    java.nio.ByteBuffer source=ie.getBytes().duplicate();byte[] bytes=new byte[source.remaining()];source.get(bytes);
                    Object decoded;
                    try{decoded=new JSONObject(WifiIeDecoder.decode(ie.getId(),ie.getIdExt(),bytes));}
                    catch(Exception e){decoded=Json.object("decode_error",e.getClass().getSimpleName());}
                    elements.put(Json.object("id",ie.getId(),"id_ext",ie.getIdExt(),"bytes_base64",Base64.encodeToString(bytes,Base64.NO_WRAP),"decoded",decoded));
                }
                String rawSsid=null;
                if(Build.VERSION.SDK_INT>=33 && r.getWifiSsid()!=null)rawSsid=Base64.encodeToString(r.getWifiSsid().getBytes(),Base64.NO_WRAP);
                append("wifi",ns,Json.object("ssid",r.SSID,"bssid",r.BSSID,"frequency_mhz",r.frequency,"channel_width",r.channelWidth,"center_frequency0_mhz",r.centerFreq0,"center_frequency1_mhz",r.centerFreq1,
                    ...
                    "ssid_bytes_base64",rawSsid,"information_elements",Build.VERSION.SDK_INT>=30?elements:null,"information_elements_scope","Android-reported beacon fields; not a full frame"));
            }
    ...
    private void captureNetwork(){
        if(log==null)return;
        try{JSONObject context=NetworkSnapshot.read(this);String encoded=context.toString();if(!encoded.equals(networkState)){networkState=encoded;append("status",null,Json.object("code","network_context","message","Phone network context changed","details",context));}}
        catch(Exception e){status("network_context_error","Network context unavailable",error(e));}
    }
