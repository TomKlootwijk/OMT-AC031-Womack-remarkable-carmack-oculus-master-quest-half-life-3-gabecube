# aTOMos Radio + Packet Studio for Android — 3.6.1.21
...
1. Install `../output/android/aTOMos-Radio-3.6.1.21-debug.apk` as an update. The package and signing key are unchanged; never uninstall to upgrade, because uninstalling deletes private sessions.
...
**Browse packets** opens a native searchable packet list with All / UDP / DNS / TCP / TLS filters. UDP includes DNS transported inside UDP. Tap a packet to read its source/destination IP and port, exact capture timestamp, lengths, DNS questions/answers and any supported NTP or DHCP candidate fields. A bounded 128-byte escaped ASCII/hex view displays original captured UDP bytes; printable bytes do not establish that application content is plaintext. Previous / Next pages read the original capture again, returning at most 200 matches each. Protocol filters and case-insensitive search cover every inspected packet, including late DNS records and bounded UDP previews. The total is limited to the inspected prefix when the 100,000-packet / 64 MiB limits or malformed container data end a scan. The exported summary JSON still retains its first 200 details; browsing does not depend on that summary. Each page checks the file SHA-256 before and after scanning and against the previous page. If the source changes, rows are discarded until you explicitly reload. Search requests are cancelled and generation-checked so stale results cannot replace a newer page or survive a destroyed screen. Rotation restores the selected filter, query, page and source pin. Reviewing a saved capture refreshes its report with the current decoder, so imported captures survive app upgrades and acquire new readable fields.
...
Private app files are excluded from backup and device transfer. The application has no INTERNET permission. The user controls where exported copies are stored. Session files are not automatically deleted.
