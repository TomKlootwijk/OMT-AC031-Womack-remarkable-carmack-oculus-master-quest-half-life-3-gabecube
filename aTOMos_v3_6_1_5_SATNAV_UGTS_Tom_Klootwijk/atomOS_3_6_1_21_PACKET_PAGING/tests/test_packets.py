...
class PacketPolicyTests(unittest.TestCase):
    ...
    def test_actual_read_rechecks_byte_limit_if_stat_size_is_stale(self):
        source = FIXTURES / "synthetic_ethernet.pcap"
        real_stat = Path.stat
        def stale_stat(path, *args, **kwargs):
            ...
            if path == source:
                parts = list(value)
                parts[6] = 1  # File grew after a one-byte metadata observation.
                return os.stat_result(parts)
            return value
        with tempfile.TemporaryDirectory() as d, patch.object(Path, "stat", new=stale_stat):
            output = Path(d) / "new"
            with self.assertRaisesRegex(ValueError, "byte limit"):
                study_capture(source, output, max_capture_bytes=50)
    ...
    def test_udp_page_arguments_are_validated_before_output_creation(self):
        bad = ({"udp_offset": -1}, {"udp_offset": True}, {"udp_offset": 1.5},
               {"udp_limit": 0}, {"udp_limit": 201}, {"udp_limit": True}, {"udp_limit": "2"})
        with tempfile.TemporaryDirectory() as d:
            for options in bad:
                with self.subTest(options=options), self.assertRaisesRegex(ValueError, "udp_"):
                    study_capture(FIXTURES / "synthetic_ethernet.pcap", Path(d) / "new", **options)
                self.assertFalse((Path(d) / "new").exists())

    def test_udp_offset_counts_matches_not_absolute_frames(self):
        rows = [{"frame.number": str(i), "frame.protocols": "ip:udp:dns" if i % 2 else "ip:tcp",
                 "udp.srcport": "53" if i % 2 else ""} for i in range(1, 10)]
        with tempfile.TemporaryDirectory() as d:
            meta = write_udp_report(rows, {}, Path(d), udp_offset=2, udp_limit=2)
            entries = json.loads((Path(d) / "udp_previews.json").read_text())["packets"]
            self.assertEqual([x["frame"] for x in entries], ["5", "7"])
            self.assertEqual(meta["total_matches"], 5)
            self.assertEqual(meta["next_offset"], 4)
...
class TsharkIntegrationTests(unittest.TestCase):
    ...
    def test_three_udp_pages_cover_all_matches_once_with_original_sha(self):
        all_frames = []
        source = (FIXTURES / "synthetic_paging_750.pcap").read_bytes()
        for offset, count, first, last in ((0, 200, 1, 299), (200, 200, 301, 599), (400, 100, 601, 749)):
            with self.subTest(offset=offset):
                summary, _, output = self.study("synthetic_paging_750.pcap", udp_offset=offset)
                self.assertEqual(summary["status"], "decoded")
                self.assertEqual(summary["decoded_packet_rows"], 750)
                meta = summary["udp_readable"]
                entries = json.loads((output / "udp_previews.json").read_text())["packets"]
                frames = [int(x["frame"]) for x in entries]
                self.assertEqual((len(frames), frames[0], frames[-1]), (count, first, last))
                self.assertEqual(meta["total_matches"], 500)
                self.assertEqual(meta["has_next"], offset < 400)
                self.assertEqual(meta["next_offset"], offset + 200 if offset < 400 else None)
                self.assertTrue(all(x["payload_available"] for x in entries))
                self.assertEqual(summary["input_sha256"], hashlib.sha256(source).hexdigest())
                self.assertEqual((output / "source_original.pcap").read_bytes(), source)
                self.assertFalse((output / "tshark_udp_payloads.stdout.txt").exists())
                all_frames.extend(frames)
                if offset == 400:
                    self.assertEqual(entries[-1]["dns_question"], "page0749.example.test")
        self.assertEqual(all_frames, [i for i in range(1, 751) if i % 3])

    def test_udp_offset_at_end_and_packet_limit_prefix_are_explicit(self):
        for options, total, status in (({"udp_offset": 500}, 500, "decoded"),
                ({"udp_offset": 400, "packet_limit": 450}, 300, "packet_limit_reached")):
            with self.subTest(options=options):
                summary, _, output = self.study("synthetic_paging_750.pcap", **options)
                self.assertEqual(summary["status"], status)
                meta = summary["udp_readable"]
                self.assertEqual(meta["total_matches"], total)
                self.assertEqual(meta["displayed_records"], 0)
                self.assertFalse(meta["has_next"])
                self.assertIsNone(meta["next_offset"])
                self.assertNotIn("tshark_udp_payloads", [x["name"] for x in summary["commands"]])
                self.assertEqual(json.loads((output / "udp_previews.json").read_text())["packets"], [])

    def test_cli_single_match_page_finds_dns_after_global_frame_450(self):
        with tempfile.TemporaryDirectory() as d:
            output = Path(d) / "study"
            command = [sys.executable, str(FIXTURES.parents[1] / "tools/study_packets.py"),
                str(FIXTURES / "synthetic_paging_750.pcap"), str(output),
                "--udp-offset", "301", "--udp-limit", "1", "--label", "SYNTHETIC CLI page"]
            result = subprocess.run(command, capture_output=True, shell=False, timeout=30)
            self.assertEqual(result.returncode, 0, result.stderr.decode())
            entries = json.loads((output / "udp_previews.json").read_text())["packets"]
            self.assertEqual(len(entries), 1)
            self.assertEqual(entries[0]["frame"], "452")
            self.assertEqual(entries[0]["dns_question"], "page0452.example.test")
            meta = json.loads(result.stdout)["udp_page"]
            self.assertEqual(meta["next_offset"], 302)
