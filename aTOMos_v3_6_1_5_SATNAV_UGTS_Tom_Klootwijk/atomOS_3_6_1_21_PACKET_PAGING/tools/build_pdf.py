"""Build the editable R21 supplement and retain the exact reviewed R20 pages."""
...
PARENT = ROOT.parent / 'atomOS_3_6_1_20_RADIO_PACKET_STUDIO'
PARENT_PDF = PARENT / 'output/pdf/aTOMos_v3_6_1_20_Radio_Packet_Studio_Tom_Klootwijk.pdf'
if not PARENT_PDF.exists():
    PARENT_PDF = ROOT / 'source/parent_R21.pdf'
PARENT_SHA256 = 'afebe08e91feb4d309f2c68268b435e0ca13a02c803264e96c2782972239a7ec'
NAME = 'aTOMos_v3_6_1_21_Packet_Paging_Tom_Klootwijk.pdf'
...
def main():
    if sha(PARENT_PDF) != PARENT_SHA256:
        raise ValueError('R20 parent PDF hash mismatch')
    fonts()
    source = ROOT / 'formal/PACKET_PAGING_PROFILE.md'
    input_hash = sha(source)
    out = ROOT / 'output/pdf'
    scratch = ROOT / 'tmp/pdf'
    out.mkdir(parents=True, exist_ok=True)
    scratch.mkdir(parents=True, exist_ok=True)
    supplement = scratch / 'r21_supplement.pdf'
    ...
    def page_frame(canvas, doc):
        ...
        canvas.drawString(48, 809, 'aTOMos  /  PACKET PAGING  /  3.6.1.21')
        canvas.setFont('Body', 8)
        canvas.drawString(48, 27, 'R21 supplement | Tom Klootwijk | 2026-09-17')
        canvas.drawRightString(547, 27, str(doc.page))
        canvas.restoreState()

    doc = SimpleDocTemplate(str(supplement), pagesize=A4, leftMargin=48, rightMargin=48,
                            topMargin=61, bottomMargin=47, title='aTOMos R21 Packet Paging', author='Tom Klootwijk')
    doc.build(body, onFirstPage=page_frame, onLaterPages=page_frame)
    intro = PdfReader(supplement)
    parent = PdfReader(PARENT_PDF)
    if len(intro.pages) != 3 or len(parent.pages) != 117:
    ...
    writer.add_outline_item('R21 packet paging profile', 0)
    writer.add_outline_item('Retained R20 mathematical foundation (original pagination)', len(intro.pages))
    writer.add_metadata({'/Title': 'aTOMos 3.6.1.21 - Packet Paging and Retained R20 Foundation', '/Author': 'Tom Klootwijk'})
    target = out / NAME
