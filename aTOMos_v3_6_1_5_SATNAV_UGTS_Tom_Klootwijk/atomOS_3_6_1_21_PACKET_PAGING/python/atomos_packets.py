...
def _udp_page(rows, offset, limit):
    if type(offset) is not int or offset < 0:
        raise ValueError("udp_offset must be a nonnegative integer")
    if type(limit) is not int or not 1 <= limit <= UDP_REPORT_RECORDS:
        raise ValueError(f"udp_limit must be an integer in 1..{UDP_REPORT_RECORDS}")
    udp_rows = [row for row in rows if row.get("udp.srcport") or row.get("udp.dstport")]
    return udp_rows, udp_rows[offset:offset + limit]


def write_udp_report(rows, previews, output: Path, *, label=None, udp_offset=0, udp_limit=UDP_REPORT_RECORDS):
    udp_rows, selected = _udp_page(rows, udp_offset, udp_limit)
    entries = []
    lines = ["# Readable UDP packet study", "", "Study label: " + (label or "not supplied"), "",
        f"Showing {len(selected)} of {len(udp_rows)} decoded UDP packet records at zero-based UDP offset {udp_offset}. "
        f"Page limit {udp_limit}; at most {UDP_REPORT_RECORDS} records and {UDP_PREVIEW_BYTES} preview bytes per record are displayed.", "",
        "Offsets count UDP matches in capture order, including DNS over UDP. Match totals cover decoded packets only, "
        "within the declared packet limit; they do not count an uninspected tail of the capture.", "",
    ...
    has_next = udp_offset + len(selected) < len(udp_rows)
    metadata = {"profile": "atomos.udp-readable.v2", "decoded_udp_records": len(udp_rows),
        "total_matches": len(udp_rows), "offset": udp_offset, "limit": udp_limit,
        "has_next": has_next, "next_offset": udp_offset + len(selected) if has_next else None,
        "match_scope": "Decoded packet prefix only; see packet_limit_reached and decoder_errors in summary.json.",
...
def markdown_report(summary):
    lines = ["# aTOMos R21 packet study", "", "Study label: " + (summary.get("study_label") or "not supplied"), "",
        ...
        f"UDP page offset {summary['udp_readable']['offset']}, limit {summary['udp_readable']['limit']}, "
        f"total decoded UDP matches {summary['udp_readable']['total_matches']}, "
        f"next offset {summary['udp_readable']['next_offset']}.", "",
...
def study_capture(source: Path, output: Path, *, tshark=DEFAULT_TSHARK, label=None,
                  ...
                  udp_offset=0, udp_limit=UDP_REPORT_RECORDS):
    source, output, tshark = Path(source), Path(output), Path(tshark)
    _udp_page([], udp_offset, udp_limit)
    ...
    # Recheck the actual bytes as the file may grow after the stat call.
    ...
    with tempfile.TemporaryDirectory(prefix="atomos-tshark-config-") as config:
        ...
        if version_status == 0:
            ...
            _, udp_selected = _udp_page(rows, udp_offset, udp_limit)
            udp_frames = [row["frame.number"] for row in udp_selected]
            if udp_frames:
                # Bound the extraction to the selected frames. Full UDP payload
                # field output is transient; only capped previews are retained.
                display_filter = ("udp && frame.number >= " + str(min(int(frame) for frame in udp_frames))
                                  + " && frame.number <= " + str(max(int(frame) for frame in udp_frames)))
    ...
    summary["udp_readable"] = write_udp_report(rows, udp_previews, output, label=label,
                                               udp_offset=udp_offset, udp_limit=udp_limit)
