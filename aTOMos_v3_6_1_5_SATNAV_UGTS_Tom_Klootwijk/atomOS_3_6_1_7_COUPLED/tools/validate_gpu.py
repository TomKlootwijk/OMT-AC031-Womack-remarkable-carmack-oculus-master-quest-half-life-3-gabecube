...
def main():
    p=argparse.ArgumentParser();p.add_argument('--sanitizer',action='store_true');p.add_argument('--device',type=int,default=0)
    p.add_argument('--build-dir',type=Path,help='Build directory; use a short absolute path on Windows if the source path is long')
    a=p.parse_args()
    ...
    build=a.build_dir.resolve() if a.build_dir else ROOT/'build_gpu'
    def call(args,name):
