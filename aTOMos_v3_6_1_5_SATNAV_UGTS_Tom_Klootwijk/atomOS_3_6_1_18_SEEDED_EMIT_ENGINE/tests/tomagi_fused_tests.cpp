...
void phase_boundary_cases() {
    // Cases are chosen at signed endpoints and around each source period,
    // independently of the modular lowering. All expectations use the C oracle.
    const std::vector<U32> values{0u,1u,0xffffffffu,0x80000000u,0x7fffffffu,
        4095u,4096u,4097u,0xfffff000u,0xffffefffu,
        16383u,16384u,16385u,0xffffc000u,0xffffbfffu,
        1048575u,1048576u,1048577u,0xfff00000u,0xffefffffu};
    std::vector<vm::Cell48> cells;
    for (U32 op : {4u,5u,9u,12u}) {
        const U32 flag_count=(op==4 || op==9)?4:1;
        for (U32 f=0;f<flag_count;++f) for (U32 argument:values) {
            const U32 index=U32(cells.size());auto c=cell(index,op,index);
            c.words[3]=op==4?f<<4:op==9?f:0;
            c.words[4]=argument;c.words[5]=index%31;
            cells.push_back(c);
        }
    }
    std::vector<vm::State64> initial;
    for (U32 ci=0;ci<cells.size();++ci) for (U32 value:values) {
        vm::State64 s{};s.words[0]=value;s.words[1]=~value;
        s.words[2]=s.words[3]=value;s.words[11]=ci;s.words[12]=0x871029abu^value;
        for (U32 mode=0;mode<4;++mode) {
            s.words[8]=mode;s.words[9]=0xfefeffffu;
            s.words[10]=(mode>>1)|2u; // raw2/3 plus all low-bit sign pairs
            s.words[4]=0x80000000u;s.words[5]=0xfffffffdu;
            s.words[6]=0x7fffffffu;s.words[7]=value;
            initial.push_back(s);
        }
    }
    const auto binary=encode(cells);Oracle expected(cells,initial);
    expected.advance(17);
    for (bool texture:{false,true}) {
        vm::VM reference(binary,16);reference.set_states(initial);
        reference.run_steps(17,texture);same_vm(reference,expected,"phase boundaries reference/C");
        for (U32 chunk:{1u,7u,32u,256u}) {
            vm::VM pure(binary,16);pure.set_states(initial);
            for (U32 done=0;done<17;) {const U32 n=std::min(chunk,17-done);pure.enqueue_batch(n,texture);done+=n;}
            same_vm(pure,expected,"phase boundaries fused/C");
        }
    }
    atomos::WordProfile profile;profile.x_lut=6;
    std::vector<U64> q(initial.size());
    for (std::size_t i=0;i<initial.size();++i) q[i]=U64(initial[i].words[0])|(U64(initial[i].words[4])<<32);
    Oracle coupled(cells,initial,q);coupled.advance(17,&profile,vm::WordBinding::RhoAndVrho);
    for (bool texture:{false,true}) for (U32 chunk:{7u,256u}) {
        vm::VM machine(binary,16);machine.set_states(initial);
        vm::WordFeedback feedback(machine,profile,vm::WordBinding::RhoAndVrho,q);
        feedback.run_steps_fused(17,texture,chunk);
        same_vm(machine,coupled,"injected phase boundaries/C");same_word(feedback,coupled,"phase boundaries word hold");
    }
}
...
int main() {
    try {
        pure_and_rekey(); lsys_signed_shift_cases(); phase_boundary_cases(); coupled_cases(); terminal_and_sticky(); ownership_and_composition();
        std::cout << "tomagi_fused_tests: " << checks << " checks passed; State64 words " << compared_state_words
                  ...
                  << "; canonical C plus independent scalar per-bit ASA/NA/JK; both fetch paths; chunks 1/7/32/256; all LSYS shifts and signed extremes; full phase quotient/seam boundaries\n";
        return 0;
