...
// Fixed-period Euclidean sum: all quotient/remainder intermediates fit i32.
// Retain the full signed quotient; TIME observes it in lineage, not only parity.
struct PhaseSum { int quotient; U32 remainder; };
template<unsigned P> __device__ inline PhaseSum phase_sum32(U32 a,U32 b){
    static_assert(P==12 || P==14,"canonical PHI/TIME periods only");
    constexpr U32 mask=(1u<<P)-1;
    const int qa=int(a>>P)-int(a>>31)*int(1u<<(32-P));
    const int qb=int(b>>P)-int(b>>31)*int(1u<<(32-P));
    const U32 rem=(a&mask)+(b&mask);
    return {qa+qb+int(rem>>P),rem&mask};
}
...
template<bool Tex>__device__ inline void transition(DeviceView v,State64& state,U32& fault,Receipt& receipt){
    ...
    case 4:{
        ...
        I64 raw=I64(signed_word(s.words[3]))+a0,w=floor_div(raw,PHI_N);s.words[3]=U32(raw-w*PHI_N);
#else
        const auto sum=phase_sum32<12>(s.words[3],c.words[4]);const int w=sum.quotient;s.words[3]=sum.remainder;
#endif
    ...
    case 5:{
#ifdef ATOMOS_WIDE_PHASE_REFERENCE
        I64 raw=I64(signed_word(s.words[2]))+a0,w=floor_div(raw,TIME_N);s.words[2]=U32(raw-w*TIME_N);
#else
        const auto sum=phase_sum32<14>(s.words[2],c.words[4]);const int w=sum.quotient;s.words[2]=sum.remainder;
#endif
        s.words[10]=U32(w)&1u;
    ...
    case 9:{
#ifdef ATOMOS_WIDE_PHASE_REFERENCE
        I64 rho=signed_word(s.words[0]),w=floor_div(rho,RHO_N);s.words[0]=U32(rho-w*RHO_N);U32 odd=U32(w)&1u;
        if(odd){I64 theta=signed_word(s.words[1]);s.words[1]=U32(norm((flags&1)?theta+THETA_N/2:I64(THETA_N/2)-theta,THETA_N));
            s.words[3]=U32(norm(-I64(signed_word(s.words[3])),PHI_N));
#else
        const U32 odd=(s.words[0]>>20)&1u;s.words[0]&=RHO_N-1;
        if(odd){const U32 theta=s.words[1]&(THETA_N-1);
            s.words[1]=((flags&1)?theta+THETA_N/2:THETA_N/2-theta)&(THETA_N-1);
            s.words[3]=(0u-s.words[3])&(PHI_N-1);
#endif
            s.words[8]^=1;if(flags&2)s.words[9]^=1;s.words[15]|=WRAP;
    ...
    case 12:{int sh=a1<0?0:a1>30?30:a1;
#ifdef ATOMOS_WIDE_PHASE_REFERENCE
        I64 chirality=(s.words[8]&1u)?-1:1,turn=(s.words[10]&1u)?1:-1;
        s.words[3]=U32(norm(I64(signed_word(s.words[3]))+chirality*turn*I64(a0),PHI_N));
#else
        const U32 turn=((s.words[8]^s.words[10])&1u)?c.words[4]:0u-c.words[4];
        s.words[3]=(s.words[3]+turn)&(PHI_N-1);
#endif
