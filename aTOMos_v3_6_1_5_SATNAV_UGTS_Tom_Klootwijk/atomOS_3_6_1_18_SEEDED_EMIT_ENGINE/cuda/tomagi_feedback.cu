...
template<bool Tex,bool Inject>__global__ void journal_fused_words(DeviceView vm,WordState* words,
        atomos::WordProfile profile,std::uint32_t count,detail::JournalDeviceView journal){
    const std::uint64_t lane=std::uint64_t(blockIdx.x)*blockDim.x+threadIdx.x;
    if(lane>=vm.state_count)return;const std::uint32_t i=std::uint32_t(lane);
    State64 state=vm.states[i];std::uint32_t fault=vm.errors[i];
    WordState word=words[i];Receipt receipt{};const std::uint64_t first_epoch=vm.epoch;
    for(std::uint32_t step=0;step<count;++step){
        ...
        detail::transition<Tex>(vm,state,fault,receipt);
        commit_word_state(word,fault,receipt,profile);
        detail::append_emit(journal,i,state,receipt);
    }
    vm.states[i]=state;vm.errors[i]=fault;vm.receipts[i]=receipt;words[i]=word;
}
}
...
EmitJournalChunk WordFeedback::run_journal_chunk(std::uint32_t ticks,bool texture,bool fused,std::uint64_t max_events){
    auto& p=*impl_;p.checked_view();
    // Admission/allocation failures precede injection and do not poison an
    // otherwise valid borrower. Runtime failures after this boundary do.
    detail::JournalCapture capture(p.source,ticks,max_events,p.words);
    p.timing.device_ms=p.timing.wall_ms=0;p.timing.steps=ticks;
    const auto begin=std::chrono::steady_clock::now();
    const unsigned blocks=unsigned((std::uint64_t(p.lanes)+127)/128);
    try{
        cu(cudaEventRecord(p.start));
        if(ticks&&fused){
            const auto view=p.checked_view();
            if(p.binding==WordBinding::RhoAndVrho){
                if(texture)journal_fused_words<true,true><<<blocks,128>>>(view,p.words,p.profile,ticks,capture.view());
                else journal_fused_words<false,true><<<blocks,128>>>(view,p.words,p.profile,ticks,capture.view());
            }else{
                if(texture)journal_fused_words<true,false><<<blocks,128>>>(view,p.words,p.profile,ticks,capture.view());
                else journal_fused_words<false,false><<<blocks,128>>>(view,p.words,p.profile,ticks,capture.view());
            }
            cu(cudaGetLastError());p.source.advance_fused_epoch(view.epoch,ticks);p.expected_epoch=p.source.epoch();
        }else for(std::uint32_t i=0;i<ticks;++i){
            const auto view=p.checked_view();
            if(p.binding==WordBinding::RhoAndVrho){inject_words<<<blocks,128>>>(view,p.words);cu(cudaGetLastError());}
            p.source.enqueue_step(texture);p.expected_epoch=p.source.epoch();
            commit_words<<<blocks,128>>>(p.source.device_view(),p.words,p.profile);cu(cudaGetLastError());
            detail::launch_journal_receipt(p.source.device_view(),capture.view());
        }
        cu(cudaEventRecord(p.stop));cu(cudaEventSynchronize(p.stop));float ms=0;
        cu(cudaEventElapsedTime(&ms,p.start,p.stop));p.timing.device_ms=ms;
        auto result=capture.finish(p.source,p.words,p.binding,p.profile);
        p.timing.wall_ms=std::chrono::duration<double,std::milli>(std::chrono::steady_clock::now()-begin).count();
        return result;
    }catch(...){p.poisoned=true;throw;}
}
