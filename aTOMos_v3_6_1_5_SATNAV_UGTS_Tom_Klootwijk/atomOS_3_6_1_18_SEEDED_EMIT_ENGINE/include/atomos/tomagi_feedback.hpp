...
public:
    ...
    // Same exact final state/receipt as run_steps, with no intermediate
    // observer inside a chunk. chunk_steps must be1..256 (default32).
    // This launch-size limit concerns responsiveness, not the word algebra.
    void run_steps_fused(std::uint32_t count,bool texture=true,std::uint32_t chunk_steps=32);
    // Synchronous opt-in complete per-lane EMIT stream for one chunk0..256.
    // Capacity admission and source binding precede any injection/transition.
