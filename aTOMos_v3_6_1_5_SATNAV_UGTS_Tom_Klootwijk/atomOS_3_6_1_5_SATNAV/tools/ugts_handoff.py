...
def process(input_dir,run_dir):
    ...
    for row in rows:
        ...
        seen.add(eid)
        record={'version':'3.6.1.5','epoch_id':eid,'t_gpst_s':t,'solver_status':row['status'],
    ...
    summary={'version':'3.6.1.5','events':events,'head_sha256':head,'scope':'UGTS computational event criteria only. Supplied bounds are not GNSS integrity/protection guarantees.'}
