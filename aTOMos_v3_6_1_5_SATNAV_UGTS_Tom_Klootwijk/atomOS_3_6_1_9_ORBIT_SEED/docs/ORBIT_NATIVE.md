...
Release3.6.1.9 adds `orbit_worker --model FILE --backend cpu|cuda [--device N]`.
...
declared work limit have distinct statuses. No trajectory is extrapolated through
...
The validated CUDA 12.8.61 build on the RTX 5070 Ti Laptop GPU reports 208
registers and 216 local bytes per thread, zero static shared bytes, and a
3,040-byte model parameter. These are measured compiler/device properties,
...
The frozen R2 validation in `results/orbit_native_r2_validation_final` passed
