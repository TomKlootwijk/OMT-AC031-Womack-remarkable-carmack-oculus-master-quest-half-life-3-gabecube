...
from orbit_seed import feedback_luts
from orbit_dynamics import validate_model


def numeric_model(model):
    ...
    lines = ["ORBIT-SEED-NATIVE-2" if r2 else "ORBIT-SEED-NATIVE-1"]
    def add(values):
...
class NativeOrbit:
    def __init__(self, worker_path, model, backend="cpu"):
        self.model, self.backend = validate_model(model), backend
        ...
        self._model_path.write_text(numeric_model(model), encoding="ascii")
    ...
    def _time(self, time_s):
        time_s = float(time_s)
        if not math.isfinite(time_s) or not self.model["domain_s"][0] <= time_s <= self.model["domain_s"][1]:
    ...
    def transition(self, feedback, q, drive):
        ...
        values = [q, drive, feedback["present"]]
        for masks in feedback["sets"]:
            values.extend(masks[k] for k in ("asa_mask", "na_mask", "boundary_mask"))
        values.extend(luts[k] for k in ("x_lut", "j_lut", "k_lut"))
