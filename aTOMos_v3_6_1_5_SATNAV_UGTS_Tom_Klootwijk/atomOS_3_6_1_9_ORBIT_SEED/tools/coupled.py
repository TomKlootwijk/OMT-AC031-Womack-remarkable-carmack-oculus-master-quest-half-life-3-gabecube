#!/usr/bin/env python3
"""Compile editable CGK-R1 equations, run independent mechanics replay, verify native traces."""
...
from coupled import PROFILE, VERSION, file_digest, load_document, replay, write_compiled
...
def run(args):
    ...
    (args.out / "model.json").write_bytes(args.input.read_bytes())
    report = {"version": VERSION, "profile": PROFILE, "status": "failed", "input_sha256": file_digest(args.input),
              "compiled_sha256": {path.name: file_digest(path) for path in compiled.glob("*.csv")},
    ...
    try:
        ...
        if args.binary is not None:
            ...
            actual = native / "trace.jsonl"
        elif args.verify is not None:
            actual = args.verify / "trace.jsonl"
        report.update(replay(trajectories, trace_path=args.out / "reference_trace.jsonl", actual_path=actual))
        report["reference_trace_sha256"] = file_digest(args.out / "reference_trace.jsonl")
...
def main():
    ...
    try:
        ...
        print(f"NOT_RUN: {exc}", file=sys.stderr); return 3
