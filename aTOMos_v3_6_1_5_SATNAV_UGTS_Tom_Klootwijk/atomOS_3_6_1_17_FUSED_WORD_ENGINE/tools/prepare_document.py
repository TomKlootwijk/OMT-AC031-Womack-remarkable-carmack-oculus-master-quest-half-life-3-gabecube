#!/usr/bin/env python3
"""Verify R17 sources and authored TeX dependencies without rewriting any file."""
...
NAME = 'aTOMos_v3_6_1_17_Fused_Word_Engine_Tom_Klootwijk'
REQUIRED_CHAPTERS = (
...
def verify_release_sources():
    ...
    if binding['version'] != '3.6.1.17':
        raise ValueError('Source binding does not identify R17')
    seen = set()
...
def document_dependencies():
    ...
    if missing:
        raise ValueError('Master omits required R17/inherited chapters: ' + ', '.join(missing))
    if '3.6.1.17' not in tex_body(docs / 'unified.tex'):
        raise ValueError('Authored master does not identify R17')
    return sorted(found)
