...
class Engine:
    ...
    def affine_plane_binding(self) -> dict:
        """Admit the R16 fixed rational plane graph; return its literal binding.

        This is deliberately a recognized graph shape, not a sampled linearity
        test. Existing evaluate/step and the R15 envelope remain unchanged.
        """
        guard = self._nodes[self._seed["guard"]]
        if guard["op"] != "plane_guard" or len(guard.get("args", [])) != 3:
            raise UnsupportedOperation("certified events require a literal fixed plane graph")
        point, normal, offset = (self._nodes[x] for x in guard["args"])
        if point["op"] != "input" or normal["op"] != "literal" or offset["op"] != "literal":
            raise UnsupportedOperation("certified plane must read one point input and literal normal/offset")
        key = point.get("key", point["id"])
        typ = point.get("type", {})
        if not isinstance(key, str) or not key or key == "__certified_affine_crossing__":
            raise DomainError("invalid certified plane input key")
        if typ.get("kind") != "vec3" or tuple(typ.get("dim", [])) != LENGTH:
            raise DomainError("certified plane point must be a length vector")
        frame = typ.get("frame")
        if not isinstance(frame, str) or not frame:
            raise DomainError("certified plane requires a nominal frame")
        # The normal and offset types, nonzero normal, and require_sdf premise
        # are also checked by the existing evaluator, using the same graph.
        self.evaluate({key: [0, 0, 0]})
        def rational(value):
            v = Phi.of(value)
            if v.b != 0:
                raise UnsupportedOperation("R16 affine-event plane coefficients must be rational")
            return v.a
        n = tuple(rational(x) for x in normal["value"])
        d = rational(offset["value"])
        return {"input_key": key, "frame": frame, "normal": n, "offset": d,
                "seed_hash": "sha256:" + hashlib.sha256(canonical_bytes(self._seed)).hexdigest()}

    def step_certified_crossing(self, event_id: str, *, origin, velocity, epoch,
                                time, frame: str, source_epoch: str,
                                sequence=None) -> StepResult:
        """Commit an exact, isolated crossing of the bound literal plane.

        The low-level method proves the submitted affine path crosses this
        plane. EventEngine additionally proves this path belongs to its current
        world epoch and that the event set is earliest and complete. No epsilon,
        new chart orientation, or change to ordinary boundary ownership occurs.
        """
        trace = {}
        try:
            if not isinstance(event_id, str) or not event_id:
                raise ValueError("nonempty immutable event identity required")
            if (not isinstance(source_epoch, str) or not source_epoch.startswith("sha256:")
                    or len(source_epoch) != 71
                    or any(c not in "0123456789abcdef" for c in source_epoch[7:])):
                raise ValueError("canonical source epoch hash required")
            binding = self.affine_plane_binding()
            if frame != binding["frame"]:
                raise DomainError("certified path frame differs from the literal plane")
            if not isinstance(origin, (list, tuple)) or not isinstance(velocity, (list, tuple)) or len(origin) != 3 or len(velocity) != 3:
                raise ValueError("certified affine path requires three origin/rate coordinates")
            p, v = tuple(_fraction(x) for x in origin), tuple(_fraction(x) for x in velocity)
            start, root = _fraction(epoch), _fraction(time)
            if root <= start:
                raise DomainError("certified crossing must be strictly after its source epoch")
            point = tuple(x + rate * (root - start) for x, rate in zip(p, v))
            slope = sum((n * rate for n, rate in zip(binding["normal"], v)), Fraction(0))
            if slope == 0:
                raise DomainError("an isolated affine crossing requires nonzero slope")
            sample = {binding["input_key"]: list(point), "__certified_affine_crossing__": {
                "profile": "ATOMOS-CERTIFIED-PLANE-CROSSING-R1", "origin": list(p),
                "velocity": list(v), "epoch": start, "time": root, "frame": frame,
                "source_epoch": source_epoch, "plane_seed_hash": binding["seed_hash"]}}
            request = canonical_bytes({"sample": sample, "crossing": True,
                                       "seam": None, "sequence": sequence}).hex()
            if event_id in self._events:
                if self._events[event_id] != request:
                    raise ValueError("same event identity with altered certified path")
                return StepResult("DUPLICATE", copy.deepcopy(self._state), {"accepted_event": False})
            logical_sequence = self._state["last_sequence"] + 1 if sequence is None else sequence
            if type(logical_sequence) is not int or logical_sequence <= self._state["last_sequence"]:
                raise ValueError("out-of-order event sequence")
            guard = self.evaluate(sample)
            if guard.data != Phi(0):
                raise DomainError("submitted root does not zero the actual guard graph")
            orientation = self._state["orientation"]
            post_side = ((slope > 0) - (slope < 0)) * ((-1) ** orientation)
            drive_mask = 1 << self._seed.get("drive_lane", 0)
            drive = drive_mask if post_side < 0 else 0
            old_q = self._state["q"]
            env = {"q": old_q, "d": drive, "e": drive_mask}
            x = eval_word(self._seed["equations"]["x"], env, self.width)
            first = asa_na(x, self.present, self._seed["masks"][0])
            second = asa_na(first["output"], self.present, self._seed["masks"][1])
            env["y"] = second["output"]
            J = eval_word(self._seed["equations"]["j"], env, self.width)
            K = eval_word(self._seed["equations"]["k"], env, self.width)
            # No seam was traversed: the optional seam lane holds exactly as
            # ordinary step does for a non-seam event.
            seam_lane = self._seed.get("seam_lane")
            if seam_lane is not None:
                seam_mask = 1 << seam_lane
                J, K = J & ~seam_mask, K & ~seam_mask
            next_q = ((J & (self.word_mask ^ old_q)) | ((self.word_mask ^ K) & old_q)) & self.present
            new_state = copy.deepcopy(self._state)
            new_state.update(q=next_q, branch_parity=self._state["branch_parity"] ^ 1,
                             last_side=post_side, last_value=guard.data,
                             step=self._state["step"] + 1, last_sequence=logical_sequence)
            new_state["events"].append({"id": event_id, "request": request})
            trace = {"profile": "ATOMOS-CERTIFIED-PLANE-CROSSING-R1", "time": root,
                     "source_epoch": source_epoch, "plane_seed_hash": binding["seed_hash"],
                     "guard": guard.data, "slope": slope, "pre_side": -post_side,
                     "post_side": post_side, "accepted_event": True,
                     "before": old_q, "drive": drive, "x": x, "stages": [first, second],
                     "output": second["output"], "j": J, "k": K, "after": next_q,
                     "orientation_before": orientation, "orientation_after": orientation,
                     "seam_parity": 0, "dimension": guard.dimension, "is_sdf": guard.is_sdf}
            self._state, self._events[event_id] = new_state, request
            return StepResult("VALUE", copy.deepcopy(new_state), trace)
        except UnsupportedOperation as error:
            return StepResult("UNKNOWN", copy.deepcopy(self._state), trace, str(error))
