...
struct Hop {u32 hop=0,program_id=0,version=0,input=0,output=0,next_slot=0,gate_evaluations=0,valid=0;u64 physical_slot=0,program_bit_reads=0;u32 page_sweep_checksum=0;u64 program_word_fetches=0;};
struct BankResult {
 u32 hops=0,error=0,sm_before=0,sm_after=0,final_program_id=0,final_input=0;
 u64 program_bit_reads=0,operator_gate_reads=0,operator_sweep_reads=0,program_sweep_reads=0,program_word_fetches=0;
 uint4 operator_warm{},operator_reread{};u32 program_warm=0,program_reread=0;
};
static_assert(sizeof(Origin)==8&&sizeof(Hop)==64,"program bank native trace ABI");
...
__device__ inline u32 page_field(const SegmentView*segments,u64 slots_per_segment,u64 slot,Shape shape,Layout layout,Origin origin,u32 offset,u32 bits,u64&reads,u64&fetches){
 u32 value=0;for(u32 done=0;done<bits;){const u64 bit=u64(offset)+done;
  ...
  // A field may cross a physical word or the seeded logical-row seam.
  // Both limits matter: the latter can change radial row inside one angular word.
  u32 take=bits-done;const u32 word_room=32-pos.angle%32,row_room=shape.angles-u32(bit%shape.angles);
  if(take>word_room)take=word_room;if(take>row_room)take=row_room;
  const u32 mask=take==32?0xffffffffu:(1u<<take)-1u;value|=((word>>(pos.angle%32))&mask)<<done;
  done+=take;reads+=take;++fetches;
 }return value;
}
...
__global__ void atomos_program_bank(const SegmentView*segments,u32 segment_count,u64 slots_per_segment,const Origin*origins,
 Shape page_shape,Layout layout,cudaTextureObject_t operators,u32 distinct,u64 pool_slots,u64 replica_stride,u32 initial_slot,u32 input_initial,u32 hops,u32 domain,u32 full_sweep,u32 inject,
 ...
 for(u32 hop=0;hop<hops;++hop){
  if(domain){current=initial_slot;input=hop;}
  ...
  u64 bit_reads=0,word_fetches=0;
  const u32 magic=page_field(segments,slots_per_segment,slot,page_shape,layout,origin,0,32,bit_reads,word_fetches);
  const u32 schema=page_field(segments,slots_per_segment,slot,page_shape,layout,origin,32,32,bit_reads,word_fetches);
  const u32 inputs=page_field(segments,slots_per_segment,slot,page_shape,layout,origin,64,32,bit_reads,word_fetches);
  const u32 outputs=page_field(segments,slots_per_segment,slot,page_shape,layout,origin,96,32,bit_reads,word_fetches);
  const u32 gates=page_field(segments,slots_per_segment,slot,page_shape,layout,origin,128,32,bit_reads,word_fetches);
  const u32 width=page_field(segments,slots_per_segment,slot,page_shape,layout,origin,160,32,bit_reads,word_fetches);
  const u32 length=page_field(segments,slots_per_segment,slot,page_shape,layout,origin,192,32,bit_reads,word_fetches);
  const u32 id=page_field(segments,slots_per_segment,slot,page_shape,layout,origin,224,32,bit_reads,word_fetches);
  record.version=page_field(segments,slots_per_segment,slot,page_shape,layout,origin,256,32,bit_reads,word_fetches);
  record.next_slot=page_field(segments,slots_per_segment,slot,page_shape,layout,origin,288,32,bit_reads,word_fetches);
  const u32 data_start=page_field(segments,slots_per_segment,slot,page_shape,layout,origin,320,32,bit_reads,word_fetches);
  const u32 reserved=page_field(segments,slots_per_segment,slot,page_shape,layout,origin,352,32,bit_reads,word_fetches);
  if(magic!=P::PAGE_MAGIC||schema!=1||inputs>32||!outputs||outputs>32||gates>P::MAX_WIRES||u64(inputs)+1+gates>P::MAX_WIRES||!width||width>10||
   ...
   record.program_bit_reads=bit_reads;record.program_word_fetches=word_fetches;trace[hop]=record;result.program_bit_reads+=bit_reads;result.program_word_fetches+=word_fetches;result.error=1;break;
  }
  ...
  for(u32 gate=0;gate<gates;++gate){
   const u32 left=page_field(segments,slots_per_segment,slot,page_shape,layout,origin,offset,width,bit_reads,word_fetches);
   const u32 right=page_field(segments,slots_per_segment,slot,page_shape,layout,origin,offset+width,width,bit_reads,word_fetches);offset+=2*width;
  ...
  if(valid)for(u32 bit=0;bit<outputs;++bit){const u32 ref=page_field(segments,slots_per_segment,slot,page_shape,layout,origin,offset,width,bit_reads,word_fetches);offset+=width;
  ...
  record.valid=valid?1u:0u;record.program_bit_reads=bit_reads;record.program_word_fetches=word_fetches;trace[hop]=record;result.program_bit_reads+=bit_reads;result.program_word_fetches+=word_fetches;result.operator_gate_reads+=record.gate_evaluations;
...
struct Options {fs::path bank,atlas,out;Layout layout=Layout::linear;std::string layout_name="linear",injection="none",mode="chain";u32 device=0,input=0,hops=8,initial_slot=0;u64 pool_mib=0,reserve_mib=1536,replica_stride=1,sweep_limit_mib=1;double fraction=0;};
Options options(int argc,char**argv){Options o;for(int i=1;i<argc;++i){const std::string key=argv[i];
 if(key=="--help"){std::cout<<"atomos_program_bank --bank FILE --atlas FILE --out NEW_DIR --layout linear|morton8 --mode chain|domain --input UINT32 --initial-slot N --hops N --pool-mib N --reserve-mib N --fill-free-fraction 0..0.90 --replica-stride N --sweep-limit-mib N --inject none|program|output\nDomain mode enumerates every input of the selected capsule (at most 12 input bits); it overrides input/hops. Immutable packed NOR pages switch through their texture-read next_slot field. Pool replicas are allocation stress, not additional learned knowledge.\n";std::exit(0);}
 ...
 else if(key=="--mode"){if(value!="chain"&&value!="domain")throw std::invalid_argument("unknown execution mode");o.mode=value;}
...
u64 expected_word_fetches(const P::Capsule&c,const Shape&s){
 // Count boundaries by an independent per-bit host walk, not the CUDA chunk formula.
 const auto count=[&](u32 offset,u32 bits){u64 reads=0,previous_cell=UINT64_MAX,previous_row=UINT64_MAX;
  for(u32 i=0;i<bits;++i){const u64 b=u64(offset)+i;u32 row=u32((u64(c.origin_row)+b/s.angles)%s.rows),raw_angle=c.origin_angle+u32(b%s.angles);if(raw_angle>=s.angles)row=s.rows-1-row;
   const u64 physical_word=u64(row)*s.words+(raw_angle%s.angles)/32,logical_row=b/s.angles;
   if(physical_word!=previous_cell||logical_row!=previous_row)++reads;previous_cell=physical_word;previous_row=logical_row;
  }return reads;
 };
 u64 reads=0;for(u32 word=0;word<12;++word)reads+=count(word*32,32);
 for(u32 field=0;field<u32(c.gates.size())*2+c.output_bits;++field)reads+=count(P::HEADER_BITS+field*c.ref_bits,c.ref_bits);return reads;
}
} // program_detail

int main(int argc,char**argv){using namespace program_detail;fs::path staging;try{
 Options o=options(argc,argv);if(fs::exists(o.out))throw std::invalid_argument("output directory exists");
 ...
 if(o.mode=="domain"){const u32 bits=bank.capsules[o.initial_slot].input_bits;if(bits>12)throw std::invalid_argument("exhaustive domain mode supports at most 12 input bits");o.hops=1u<<bits;o.input=0;}
 ...
 for(const auto&segment:pool)for(u64 offset=0;offset<segment.slots*page_words;offset+=audit_chunk_words){const u64 count=std::min(audit_chunk_words,segment.slots*page_words-offset);
  ...
  const u64 first=segment.first_slot*page_words+offset;for(u64 i=0;i<count;){const u64 source=(first+i)%template_words,span=std::min(count-i,template_words-source);
   if(!std::equal(audit.begin()+std::ptrdiff_t(i),audit.begin()+std::ptrdiff_t(i+span),templates.begin()+std::ptrdiff_t(source)))throw std::runtime_error("pool initialization audit mismatch");i+=span;}audited+=count*4;}
 ...
 atomos_program_bank<<<1,32>>>(segment_buffer.get(),segment_count,slots_per_segment,origin_buffer.get(),s,o.layout,operator_texture.get(),distinct,pool_slots,o.replica_stride,o.initial_slot,o.input,o.hops,o.mode=="domain"?1:0,full_sweep?1:0,o.injection=="output"?2:0,wire_buffer.get(),trace_buffer.get(),result_buffer.get());
 ...
 u32 expected_program=o.initial_slot,expected_input=o.input;u64 expected_reads=0,expected_gates=0,expected_fetches=0;std::string reason;std::vector<u64> visited_slots;
 for(u32 hop=0;hop<o.hops;++hop){if(o.mode=="domain"){expected_program=o.initial_slot;expected_input=hop;}const auto&c=bank.capsules[expected_program];const auto&actual=traces[hop];const u32 output=P::evaluate(c,expected_input);
  ...
  const u64 fetches=expected_word_fetches(c,s);u32 page_checksum=0;if(!full_sweep)for(u64 word=0;word<page_words;++word)page_checksum^=templates[std::size_t(u64(expected_program)*page_words+word)];
  ...
  if(actual.page_sweep_checksum!=page_checksum||actual.program_word_fetches!=fetches)verified=false;
  expected_reads+=reads;expected_fetches+=fetches;expected_gates+=c.gates.size();expected_program=c.next_slot;expected_input=output;
 }
 if(result.final_program_id!=expected_program||result.final_input!=expected_input||result.program_bit_reads!=expected_reads||result.program_word_fetches!=expected_fetches||result.operator_gate_reads!=expected_gates||result.operator_sweep_reads!=128)verified=false;
 ...
 const u64 attempted=std::min(u64(o.hops),u64(result.hops)+(result.error?1:0));for(u64 i=0;i<attempted;++i)visited_slots.push_back(traces[std::size_t(i)].physical_slot);
 ...
 {auto out=file(staging/"trace.csv");out<<"hop,program_id,version,physical_slot,input,output,next_slot,gate_evaluations,program_bit_reads,valid,page_sweep_checksum,program_word_fetches\n";
  for(const auto&r:traces)out<<r.hop<<','<<r.program_id<<','<<r.version<<','<<r.physical_slot<<','<<r.input<<','<<r.output<<','<<r.next_slot<<','<<r.gate_evaluations<<','<<r.program_bit_reads<<','<<r.valid<<','<<r.page_sweep_checksum<<','<<r.program_word_fetches<<'\n';}
 std::ostringstream report;report<<std::setprecision(17)<<"{\"schema\":\"atomOS-program-bank-runtime-v1\",\"concept_author\":\"Tom Klootwijk\",\"status\":"<<json_string(verified?"passed":"rejected_verification")<<",\"accepted\":"<<(verified?"true":"false")<<",\"committed\":"<<(verified?"true":"false")
  <<",\"reason\":"<<json_string(reason)<<",\"encoding\":"<<json_string(P::ENCODING)<<",\"layout\":"<<json_string(o.layout_name)<<",\"execution_mode\":"<<json_string(o.mode)<<",\"topology\":\"klein_m1_angular_twist\",\"coordinate_chart\":\"rho=log(r), angular nodes\",\"injection\":"<<json_string(o.injection)
  ...
  <<",\"program_word_fetches\":"<<result.program_word_fetches<<",\"program_sweep_reads\":"<<result.program_sweep_reads<<",\"operator_sweep_reads\":"<<result.operator_sweep_reads<<",\"operator_warm\":"<<checksum(result.operator_warm)<<",\"operator_reread\":"<<checksum(result.operator_reread)<<",\"program_warm\":"<<result.program_warm<<",\"program_reread\":"<<result.program_reread
