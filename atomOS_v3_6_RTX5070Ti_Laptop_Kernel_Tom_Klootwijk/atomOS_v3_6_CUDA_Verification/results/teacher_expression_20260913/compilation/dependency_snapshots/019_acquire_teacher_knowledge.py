...
def response_format(profile):
    """Constrain syntax and interface only; no correct algorithm is supplied."""
    ref = {'$ref': '#/$defs/expression'}
    def tuple_array(items):
        return {'type': 'array', 'prefixItems': items, 'minItems': len(items), 'maxItems': len(items)}
    forms = [tuple_array([{'const': 'input'}, {'type': 'integer', 'minimum': 0, 'maximum': profile['input_bits'] - 1}]),
             tuple_array([{'const': 'constant'}, {'enum': [0, 1]}]), tuple_array([{'const': 'not'}, ref])]
    forms.extend(tuple_array([{'const': op}, ref, ref]) for op in ('and', 'or', 'xor'))
    procedure = {'type': 'object', 'additionalProperties': False,
                 'properties': {'name': {'const': profile['profile_id']}, 'input_bits': {'const': profile['input_bits']},
                                'output_bits': {'const': profile['output_bits']},
                                'outputs': {'type': 'array', 'minItems': profile['output_bits'],
                                            'maxItems': profile['output_bits'], 'items': ref}},
                 'required': ['name', 'input_bits', 'output_bits', 'outputs']}
    return {'type': 'object', 'additionalProperties': False, '$defs': {'expression': {'anyOf': forms}},
            'properties': {'schema': {'const': 'atomos-teacher-procedure-v1'},
                           'profile_id': {'const': profile['profile_id']},
...
def acquire(*, curriculum, teachers, out, max_tokens=2048, frontend='boolean-tree'):
    ...
    expression_frontend = None
    if frontend == 'boolean-expression':
        import teacher_expression_frontend as expression_frontend
    frozen = (expression_frontend.load_curriculum if expression_frontend else load_curriculum)(curriculum)
    manifest = frozen['manifest']
    config_bytes = teachers.read_bytes()
    ...
    if expression_frontend:
        path = ROOT / 'python/teacher_expression_frontend.py'
        bound[str(path)] = sha(path)
    if bound[str(teachers)] != hashlib.sha256(config_bytes).hexdigest():
        raise ValueError('teacher configuration changed between parsing and freezing')
    if bound[str(curriculum / 'curriculum.json')] != frozen['curriculum_sha256']:
        raise ValueError('curriculum changed between validated load and freezing')
    for profile in manifest['profiles']:
        for file_key, hash_key in (('prompt_file', 'prompt_sha256'), ('oracle_file', 'oracle_sha256')):
            if bound[str(curriculum / profile[file_key])] != profile[hash_key]:
                raise ValueError('frozen curriculum artifact changed between validated load and binding')
    ...
    report = {'schema': 'atomos-local-teacher-acquisition-v1', 'status': 'running',
              'started_at_utc': now(), 'runtime': version, 'endpoint': endpoint, 'frontend': frontend,
        ...
        for teacher in models:
            ...
            try:
                for profile in manifest['profiles']:
                    ...
                    syntax = expression_frontend.response_format(profile_id) if expression_frontend else response_format(profile)
                    request = {'model': teacher['ollama_model'], 'stream': False, 'format': syntax,
                    ...
                    run.update(schema='atomos-teacher-inference-run-v1', profile_id=profile_id, frontend=frontend,
...
def main():
    ...
    parser.add_argument('--frontend', choices=('boolean-tree', 'boolean-expression'), default='boolean-tree')
    args = parser.parse_args()
    report = acquire(curriculum=args.curriculum, teachers=args.teachers, out=args.out,
                     max_tokens=args.max_tokens, frontend=args.frontend)
