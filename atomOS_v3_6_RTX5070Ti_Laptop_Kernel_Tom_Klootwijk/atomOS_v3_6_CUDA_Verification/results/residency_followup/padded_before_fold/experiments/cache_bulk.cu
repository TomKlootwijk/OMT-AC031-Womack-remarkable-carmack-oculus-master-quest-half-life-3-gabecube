...
void print_ids(const std::vector<uint4>&ids){std::cout<<'[';for(std::size_t i=0;i<ids.size();i++)std::cout<<(i?",":"")<<ids[i].x;std::cout<<']';}
...
int main(int argc,char**argv){try{
 ...
 std::string mode="recurrent",profile="mixed";
 for(int a=1;a<argc;a++){
  ...
  else if(key=="--epochs")epochs=number(value);else if(key=="--seed")seed=number(value);
  else if(key=="--layout"){
 ...
 for(u32 epoch=0;epoch<epochs;epoch++){
  ...
  float ms=0;AO_CUDA(cudaEventElapsedTime(&ms,start.get(),stop.get()));verify();commit_verified(fixture,state,candidates);times.push_back(ms);
 }
 std::cout<<std::setprecision(17)<<"{\"status\":\"passed\",\"scope\":\"experimental_bulk_io_warm_work_retention_probe\",\"device\":"<<device_record(device)
 ...
 for(std::size_t i=0;i<times.size();i++)std::cout<<(i?",":"")<<times[i];std::cout<<"],\"sm_mappings\":[";
 for(std::size_t i=0;i<mappings.size();i++){
  std::cout<<(i?",":"")<<"{\"launch\":"<<i<<",\"setup\":"<<(i==0?"true":"false")<<",\"before\":";print_ids(mappings[i].before);std::cout<<",\"after\":";print_ids(mappings[i].after);std::cout<<'}';
 }
 std::cout<<"]}\n";return 0;
