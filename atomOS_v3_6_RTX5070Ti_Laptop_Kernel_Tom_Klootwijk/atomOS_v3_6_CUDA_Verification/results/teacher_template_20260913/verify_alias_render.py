...
def parameters(text):
    ...
    for line in text.splitlines():
        ...
        result.setdefault(name, []).append(decode_json(value.encode('utf-8')))
    return result

def main():
    study = ROOT / 'results/teacher_template_20260913'
    out = study / 'verified_alias_v2'
    ...
    try:
        ...
        if parameters(show['parameters']) != parameters(original['parameters']):
            raise ValueError('inherited parameter values changed')
        for key in ('model_info','system','tensors'):
        ...
        normalized_after, normalized_original = copy.deepcopy(after), copy.deepcopy(original)
        normalized_after['parameters'] = parameters(after['parameters'])
        normalized_original['parameters'] = parameters(original['parameters'])
        if normalized_after!=normalized_original: raise ValueError('original imported alias was modified')
