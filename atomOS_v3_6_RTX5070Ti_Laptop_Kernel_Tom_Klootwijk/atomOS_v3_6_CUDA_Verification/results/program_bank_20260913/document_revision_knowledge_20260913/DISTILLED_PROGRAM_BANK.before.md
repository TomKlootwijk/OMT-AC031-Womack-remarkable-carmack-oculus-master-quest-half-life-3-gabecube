...
Tom selected **general language-model behavior as the long-term target**. The
immediate engineering task is to construct and measure the proposed substrate and
an actual example-driven distiller. A small finite skill library is a milestone;
it does not establish general language-model replacement.
...
| General language-model replacement | Defined milestones and a Hugging Face teacher shortlist | Long-term target remains unachieved; tokenizer, context model and language-learning objective still need implementation |
...
## Route toward language-model behaviour
...
| Small language learner | Token/context encoding, recurrent state and next-token or sequence prediction | Define the objective and decoder; compare held-out quality and full inference cost against n-gram and small neural baselines |
...
| General language-model behaviour | Dialogue, summarization, document questions and cross-domain reasoning | Predeclared broad evaluation, contamination controls, robustness, human assessment and quality/cost curves against actual language models |
...
automatically inherited by this kernel. Actual teacher/student distillation needs
an explicit supervision signal and objective, as distinguished in
[knowledge distillation](https://arxiv.org/abs/1503.02531) and
[sequence-level distillation](https://aclanthology.org/D16-1139/).
...
Open language-design decisions include tokenization, context/state representation,
probability or deterministic-decoder semantics, teacher/corpus choice, learning
objective, search scalability, abstraction induction and how language features
bind to the chart. Universal Boolean expressiveness supplies an execution route
given resources. It does not prove efficient learning or competitive language
inference. Novelty on fixed probes is only novelty on those probes.
