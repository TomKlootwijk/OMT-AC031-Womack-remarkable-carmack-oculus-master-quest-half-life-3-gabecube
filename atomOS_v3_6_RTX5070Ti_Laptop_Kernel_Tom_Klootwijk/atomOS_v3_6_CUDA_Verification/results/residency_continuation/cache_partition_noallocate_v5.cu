...
template<bool NoAllocate> __global__ void atomos_epoch_partition_probe(Config c,const State*state,const Lane*input,
 ...
 u32*sm_before,u32*sm_after,u32 atlas_texels,u32 chunk,u32 grid_barrier,u32 retain_only,u32 word_only){
 ...
 for(u64 k=begin+tid;!retain_only&&k<end;k+=PARTITION_THREADS){
  ...
  retain_input_loads(l,s);Result value{};
...
int main(int argc,char**argv){try{
 u32 rows=128,angles=1024,epochs=3,grid_barrier=1,retain_only=0,word_only=0;bool no_allocate=false;Layout layout=Layout::linear;
 for(int a=1;a<argc;a++){
  const std::string key=argv[a];
  if(key=="--help"){
   std::cout<<"Standalone cache partition control\n--rows N --angles N --epochs 1..16 --layout linear|morton8 --barrier grid|block --control epoch|retain-only|word-only --io cg|no-allocate\n"
  ...
  if(key=="--io"){
   ...
   if(value=="epoch"){retain_only=0;word_only=0;}
   else if(value=="retain-only"){retain_only=1;word_only=0;}
   else if(value=="word-only"){retain_only=0;word_only=1;}
   else throw std::invalid_argument("control must be epoch, retain-only or word-only");
  }else if(key=="--barrier"){
 ...
 auto launch=[&](){
  ...
  void*arguments[]={&fixture.config,&state_ptr,&input_ptr,&texture_handle,&output_ptr,&pre_ptr,&post_ptr,
                    &sm_pre_ptr,&sm_post_ptr,&atlas_argument,&chunk_argument,&grid_barrier,&retain_only,&word_only};
 ...
 poison();launch();AO_CUDA(cudaDeviceSynchronize());verify_probes();if(!retain_only&&!word_only)verify_results(fixture,state,candidates);
 for(u32 epoch=0;epoch<epochs;epoch++){
  ...
  verify_probes();if(!retain_only&&!word_only)commit_verified(fixture,state,candidates);times.push_back(ms);
 }

 std::cout<<std::setprecision(17)<<"{\"status\":\"passed\",\"scope\":"<<json_string(retain_only?"experimental_partition_retention_only_control":word_only?"experimental_partition_word_only_control":"experimental_partition_warm_work_retention_probe")<<",\"device\":"<<device_record(device)
  ...
  <<",\"control\":"<<json_string(retain_only?"retain-only":word_only?"word-only":"epoch")
  ...
  <<",\"verified_lane_epochs\":"<<((retain_only||word_only)?0:n*epochs)<<",\"checksum_components\":4,\"checksum_scheme\":\"per_thread_componentwise_xor\",\"checksum_disagreements\":0"
  <<",\"candidate_verification\":"<<json_string((retain_only||word_only)?"not_run":"passed")<<",\"host_state_committed\":"<<((retain_only||word_only)?"false":"true")<<",\"sm_mapping_verified\":true,\"setup_launches\":1,\"setup_launch_verified\":"<<((retain_only||word_only)?"false":"true")<<",\"setup_checksum_verified\":true"
  <<",\"timing_scope\":"<<json_string(retain_only?"entire_warm_probe_control_including_barriers_and_diagnostic_stores":word_only?"entire_warm_word_only_probe_control_including_barriers_and_diagnostic_stores":"entire_warm_work_probe_kernel_including_barriers_and_diagnostic_stores")<<",\"compute_ms\":[";
