...
__global__ void atomos_epoch_partition_probe(Config c,const State*state,const Lane*input,
 ...
 u32*sm_before,u32*sm_after,u32 atlas_texels,u32 chunk,u32 grid_barrier){
...
int main(int argc,char**argv){try{
 u32 rows=128,angles=1024,epochs=3,grid_barrier=1;Layout layout=Layout::linear;
 for(int a=1;a<argc;a++){
  const std::string key=argv[a];
  if(key=="--help"){
   std::cout<<"Standalone cache partition control\n--rows N --angles N --epochs 1..16 --layout linear|morton8 --barrier grid|block\n"
 ...
 auto launch=[&](){
  ...
  void*arguments[]={&fixture.config,&state_ptr,&input_ptr,&texture_handle,&output_ptr,&pre_ptr,&post_ptr,
                    &sm_pre_ptr,&sm_post_ptr,&atlas_argument,&chunk_argument,&grid_barrier};
