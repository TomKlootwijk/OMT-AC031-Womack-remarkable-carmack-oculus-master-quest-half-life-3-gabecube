...
int main(int argc,char**argv){try{
 u32 rows=128,angles=1024,epochs=3,grid_barrier=1,retain_only=0,word_only=0;Layout layout=Layout::linear;
 for(int a=1;a<argc;a++){
  const std::string key=argv[a];
  if(key=="--help"){
   std::cout<<"Standalone cache partition control\n--rows N --angles N --epochs 1..16 --layout linear|morton8 --barrier grid|block --control epoch|retain-only|word-only\n"
