...
__device__ __forceinline__ u32 load_word_cg(const State*state,u64 index){
 u32 value;const u32*word=&state[index].word;
 asm volatile("ld.global.cg.u32 %0, [%1];":"=r"(value):"l"(word):"memory");return value;
}
__device__ __forceinline__ void store_word_cg(u32*target,u32 value){
 asm volatile("st.global.cg.u32 [%0], %1;"::"l"(target),"r"(value):"memory");
}
__device__ __forceinline__ u32 transport_word(const Shape&s,const State*state,u32 own,u32 r,u32 w,Carrier carrier){
 const u32 valid=valid_mask(s,w);own&=valid;
 if(carrier==Carrier::klein_plus){
  const u32 source_r=w?r:s.rows-1-r,source_w=w?w-1:s.words-1;
  const u32 bit=w?31:(s.angles-1)&31;
  return ((own<<1)|((load_word_cg(state,u64(source_r)*s.padded_words+source_w)>>bit)&1u))&valid;
 }
 if(carrier==Carrier::klein_minus){
  const bool last=w+1==s.words;const u32 source_r=last?s.rows-1-r:r,source_w=last?0:w+1;
  const u32 bit=last?(s.angles-1)&31:31;
  return ((own>>1)|((load_word_cg(state,u64(source_r)*s.padded_words+source_w)&1u)<<bit))&valid;
 }
 return own;
}
  ...
  for(u32 wave=0;wave<waves;wave++){
   ...
   if(owner==wave){
    ...
    if(carrier!=Carrier::direct){
     current.word=r<c.shape.rows&&w<c.shape.words?transport_word(c.shape,state,current.word,r,w,carrier):0;
     ...
     store_word_cg(transported_words+u64(r)*c.shape.padded_words+w,current.word);
    }
...
int main(int argc,char**argv){fs::path staging;try{
 ...
 receipt<<std::setprecision(17)<<"{\"status\":\"passed\",\"scope\":\"experimental_bulk_io_warm_work_retention_probe\",\"device\":"<<device_record(device)
  ...
  <<",\"carrier_neighbor_word_reads_per_epoch\":"<<(carrier_bytes?n:0)<<",\"carrier_word_stores_per_epoch\":"<<(carrier_bytes?atlas:0)
  <<",\"carrier_io_cache_policy\":"<<json_string(carrier_bytes?"ld.global.cg and st.global.cg":"not_applicable")
