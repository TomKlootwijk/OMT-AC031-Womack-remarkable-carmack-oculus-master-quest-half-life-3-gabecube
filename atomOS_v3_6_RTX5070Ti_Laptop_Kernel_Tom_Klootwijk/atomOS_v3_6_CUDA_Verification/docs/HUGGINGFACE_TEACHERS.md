...
Tom's subsequent clarification defines the destination as an independent
knowledge and program engine. These models are acquisition tools: their proposed
facts, rules, procedures and counterexamples are checked and represented explicitly.
Their weights and token distributions are not the target representation. See
[the architecture clarification](KNOWLEDGE_EXTRACTION_ARCHITECTURE.md).
...
The present distiller accepts finite Boolean input/output examples. The next
milestone is an explicit knowledge pack with typed assertions, rule applicability,
retained evidence, unknown/conflict states and executable reasoning. A language
interface can compile questions into those operations. Rebuilding a sequence
model is not a prerequisite. Direct extraction, program induction, abstraction
and compilation are distinct transfer operations and should be recorded as such.
...
The long-term target is broad knowledge and language capability implemented
through the independent substrate. These teachers provide acquisition material;
the quality of admitted knowledge and execution must be established independently.
