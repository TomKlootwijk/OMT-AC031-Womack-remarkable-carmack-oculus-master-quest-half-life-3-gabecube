...
int main(){try{
 ...
 group("GPU free-memory admission boundaries",[]{constexpr u64 reserve=512ull*1048576;require(!gpu_payload_admitted(0,0),"zero free rejected");require(!gpu_payload_admitted(0,reserve-1),"below reserve rejected");require(!gpu_payload_admitted(0,reserve),"reserve alone rejected");require(gpu_payload_admitted(1,reserve+1),"exact remaining reserve admitted");require(!gpu_payload_admitted(2,reserve+1),"reserve shortfall rejected");require(gpu_payload_admitted(reserve,2*reserve),"exact half and reserve admitted");require(!gpu_payload_admitted(reserve+1,2*reserve),"half-free excess rejected");require(gpu_payload_admitted(2*reserve,4*reserve),"larger exact half admitted");require(!gpu_payload_admitted(2*reserve+1,4*reserve),"half-free policy remains enforced");require(!gpu_payload_admitted(std::numeric_limits<u64>::max(),reserve+1),"huge payload rejected without subtraction underflow");});
