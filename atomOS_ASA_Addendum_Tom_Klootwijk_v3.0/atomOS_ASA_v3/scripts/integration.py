...
def verify_execution_metadata(meta, args, gpu):
    ...
    if meta['sample_order'] != options.get('--sample-order', 'natural'):
...
def main(argv=None):
    ...
    else:cases += [(['--warmup','1'],False),(['--repeat','2'],False)]
    for args,expected in cases:
        with tempfile.TemporaryDirectory(prefix='asa_case_') as tmp:
            ...
            results.append({'arguments':args,'expected_success':expected,'status':'pass',
                            **({'oracle':oracle,'gpu_executed':meta['gpu_executed'],'sample_order':meta['sample_order']} if expected else {})})
